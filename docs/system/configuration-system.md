...
- **Settings + Record UX Parity (May 2026)** — Field policy UI + PATCH enforcement for opportunity/job **enforceable** subset; layout integrity panel; Settings index parity. **Deferred:** full layout builder, policy on status/tour/quote/FKs, BOS agent config. Closeout: **`docs/sprints/05_2026/settings_record_ux_parity_sprint.md`** §12; inventories: **`docs/execution/admin-settings-config-parity.md`**.
