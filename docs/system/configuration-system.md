...
- **Placement priority (waitlist lanes)** — subtree **`placement_priority_v1`** on **`work_units.metadata`** (and optional department defaults): enable flag, preset **`profile_id`**, lane filter, evaluation cap, display flags, rule order — validated on PATCH; drives queue preview ordering only when enabled (**opt-in**). Settings UI: **`/admin/settings/placement-priority`** (**Waitlist Ranking Policy** — operator priority factors + ranking mode). Settings V2 reframe: **`docs/sprints/05_2026/waitlist_ranking_policy_settings_v2.md`**. Runtime position + ranking validation: **`docs/sprints/05_2026/waitlist_ranking_validation_position_controls.md`**.
