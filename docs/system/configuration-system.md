...
- **Needs attention buckets** — `metadata.opportunity_attention_rules.needs_attention_buckets[]` supports optional **`priority`** (lower first; fallback **`order`**) and **`icon`** (same token convention as queue icons). Defaults live in **`web/lib/opportunities/needsAttentionBuckets.ts`**.
