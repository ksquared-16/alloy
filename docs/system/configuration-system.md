...
- **Inquiry child fields (deferred):** Settings → Fields does **not** yet expose per-child participation rows. Drawer **`inquiry_children`** is an **injected_system** section with platform-hardcoded columns; OCM native fields PATCH via **`/api/admin/opportunity-customer-members/:id`**. Planned: Settings entity_type **`inquiry_child`** (backed by OCM) + narrow allowlist; see sprint audit in **`docs/sprints/05_2026/linked_record_field_editing_v1.md`**.
