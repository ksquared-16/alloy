...
- **Queue definitions** — `work_units.queue_definition` validated as v1 (`web/lib/config/queueDefinitionSchema.ts`). **Enrollment Pipeline** canonical shape for childcare enrollment ops is centralized in **`web/lib/config/enrollmentPipelineQueueDefinitionV1.ts`** so scripts (`web/scripts/ensureEnrollmentPipelineWorkUnitV1.ts`) and DB migrations stay aligned — UI pills and summaries remain **definition-driven**, not hardcoded in React.
