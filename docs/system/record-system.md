...
- **Opportunity custom fields in drawer:** `PATCH /api/admin/opportunities/:id` accepts **field_values-only** bodies (e.g. `inquiry_source`, `desired_start_date`) via `upsertFieldValuesFromBody` after native column updates. **`desired_start_date`** is inquiry/household-level in V1 (opportunity `field_values` + metadata mirror); per-child start is **not** configurable in Settings today — planned on **`inquiry_child`** surface backed by **`opportunity_customer_members`** (native column recommended; see **`docs/sprints/05_2026/linked_record_field_editing_v1.md`** § Audit — configurable Inquiry child surface).
