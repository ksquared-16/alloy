...
- **Opportunity custom fields in drawer:** `PATCH /api/admin/opportunities/:id` accepts **field_values-only** bodies (e.g. `inquiry_source`, `desired_start_date`) via `upsertFieldValuesFromBody` after native column updates.
