...
- **Drawer layout (Settings V1):** Effective layout from **`record_drawer_layouts`** / **`record_layouts`**. Opportunity **workflow v1** supports operator **`overview_section_order`**, **`overview_hidden_sections`**, and **`inquiry_workflow_sections`** title edits via Settings → Layouts (`opportunityWorkflowV1SectionConfig.ts`, `AdminEntityDrawer` + `effectiveDrawerLayoutPreview.ts`). Field grouping labels remain on **`field_section_definitions`**.
