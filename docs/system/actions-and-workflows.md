...
- **Status labels vs automation:** Display names — **Settings → Statuses**. Condition-driven status updates (e.g. tour date set → Tour Scheduled) — **Automations** / workflows; read-only reference table **Workflow automation rules** (`status_transition_rules`). Custom handlers — deferred to automation builder (`docs/sprints/05_2026/settings_control_plane_closeout.md`).
