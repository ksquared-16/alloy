...
- **Tour scheduling (enrollment CRM V1):** **`tour_bookings`** anchor org + opportunity + site (`location_id`) + absolute `start_at`/`end_at` for sales tours. **`tour_availability_rules`** define recurring bookable windows. **`public.schedules`** stays **job-attached** (`job_id` NOT NULL) for service visits — tours are **not** schedules rows. **`opportunities.metadata.tour_date` / `tour_time`** may mirror a **confirmed** booking for queue/Needs Attention compatibility; scheduling truth remains **`tour_bookings`** (see **`docs/sprints/05_2026/tour_scheduling_v1.md`**).
