...
- **Communications (canonical):** **`communication_threads`** (org + primary entity + channel + `recipient_key`) and **`communication_messages`** (inbound/outbound rows, read/unread, queue status). **`communication_provider_bindings`** selects Twilio/Resend configuration per org/channel (see **`docs/product/crm-system.md`**).
