...
| **Lifecycle** | Configurable operator grouping (Lifecycle Hub) that owns one or more **work units** and appears on **`/workspace`** landing tiles and sidebar nav. Primary navigation spine — not a CRM status field. |
| **Work unit** | Scoped **execution domain** with **`queue_definition`** (lanes/pills). Belongs to a department for ACL/metadata but operators reach it via **lifecycle → slug route** (`/workspace/work-unit/:slug`). **Not** a lifecycle stage; enrollment uses one **`enrollment_pipeline`** WU with multiple domain queues inside `queue_definition`. |
...
| **Department** | Org-scoped grouping for **access control**, metadata (attention rules, KPI rollup), and work-unit ownership. **Not** the primary operator navigation spine (use **Lifecycle** → Work Unit). Dept UUID routes (`…/dept/…`) are internal/compat. |
