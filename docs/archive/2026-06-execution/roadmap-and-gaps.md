...
- **Settings + Record UX Parity (May 2026 — complete):** Four-plane Settings control plane V1 — field policy UI + PATCH enforcement (opportunity/job enforceable subset), opportunity workflow v1 layout sections, org action placement editor, layout integrity panel, UX contract pass — **`docs/sprints/archive/05_2026/settings_record_ux_parity_sprint.md`** §12–§13, **`docs/system/configuration-system.md`**. **Deferred:** Record Experience Builder, full `record_actions` migration, ops drawer mutate, `condition_config` editors, person/location field modal parity (linked-record inline PATCH **shipped** — **`linked_record_field_editing_v1.md`**).
- **Child lifecycle + work-unit convergence (May 2026 — closed):** Grain-aware `enrollment_pipeline` v2, candidate-grain waitlist + child-grain enrollment runtime, read-only case rollups, WU filter UX, strict-mode readiness tooling — **`docs/sprints/archive/05_2026/completed/child_lifecycle_work_unit_convergence_closeout.md`**. **Deferred follow-ons:** Settings Config Management (domain/NA presentation CRUD); strict-mode activation; server-side queue search; waitlist forecasting/capacity — see closeout § future sprints and **`waitlist_orchestration_phase2_architecture.md`**.
- **Waitlist priority fact truth (May 2026 — closed):** Child site/cohort on OCM, record-sourced V2 household facts, employee person UI, candidate repair, forms intake mapping, strict-mode **checklist documented** — **`docs/sprints/archive/05_2026/waitlist_priority_fact_truth_child_scope.md`**. **`shadow_mode: false` not enabled**; site-scoped program catalog deferred.
- **Waitlist demo readiness (May 2026 — closed):** `waitlist_demo_v1` cleanup/reseed, person drawer doctrine, Settings locations hierarchy, demo walkthrough QA — **`docs/sprints/archive/05_2026/waitlist_demo_readiness_final_pass.md`**. **Waitlist ranking validation + position controls (May 2026 — closed):** runtime position on V2 candidate rows, exact manual position, `qa:waitlist:ranking` / `qa:waitlist:shadow-mode` — **`docs/sprints/archive/05_2026/waitlist_ranking_validation_position_controls.md`**.
...
1. **Enrollment Packet Phase 2** / enrollment-forms completion — **`docs/sprints/archive/05_2026/later-phase/enrollment_packet_phase_2.md`**.
...
3. **Tour scheduling Phase 2 Band B+** — **`docs/sprints/archive/05_2026/later-phase/tour_scheduling_phase_2.md`** (Band A **complete** — **`completed/tour_scheduling_phase2_band_a_closeout.md`**).
...
- **Waitlist orchestration continuation** — Phase 2 pilot → production; forecasting/capacity engine — **`docs/sprints/archive/05_2026/waitlist_orchestration_phase2_architecture.md`**.
...
- **BOS operational assist (routing + communication drafting)** — **~2026-05-21 → 2026-05-26** — deterministic handoff routing, synthesized customer-facing drafts (separate from recommendation copy), SMS/email channel bodies in review card, Review Assist placement + drawer reveal doctrine, native Work with BOS CTA — **`docs/sprints/archive/06_2026/completed/bos_assist_routing_communication_drafting_closeout.md`**.
- **Child lifecycle + work-unit convergence** — **~2026-05-27 → 2026-06-01** (migrations **`20260601100000`–`20260601150000`**, **`20260602100000`**) — `enrollment_pipeline` v2 domains/grains, OCM lifecycle SoT, candidate waitlist + child enrollment runtime, client-side WU filters — **`docs/sprints/archive/05_2026/completed/child_lifecycle_work_unit_convergence_closeout.md`**.
- **Waitlist priority fact truth + demo readiness + ranking controls** — **~2026-05-28 → 2026-05-29** — OCM site/cohort, record-sourced V2 facts, demo reseed, runtime position display + manual adjustment — sprints in **`docs/sprints/archive/05_2026/completed/`**.
- **Tour Scheduling Phase 2 Band A** — **~2026-05-27** — booking lifecycle comms/reminders via **`communication_scheduled_sends`** (`20260527150000`) — **`docs/sprints/archive/05_2026/completed/tour_scheduling_phase2_band_a_closeout.md`**.
- **Forms MVP productization** — **~2026-05-28** — operational intent templates, simplified form setup, share/embed UX — **`docs/sprints/archive/05_2026/completed/forms_mvp_productization.md`**.
- **Global Search V1** — **May 2026** — **`GET /api/admin/global-search`**, AdminV2 header search — **`docs/sprints/archive/05_2026/completed/global_search_v1_closeout.md`**.
- **Person / location drawer convergence** — **~2026-05-29 → 2026-05-30** — shared record primitives, location metadata fields, household copy — **`docs/sprints/archive/05_2026/completed/record_person_location_convergence_audit.md`** (closeout).
- **Person drawer layout runtime v1** — **~2026-05-31** — migration **`20260531140000`**, Settings read-only visibility — **`docs/sprints/archive/05_2026/person_drawer_runtime_layout_migration.md`**.
- **Lifecycle action alignment (partial closeout)** — **~2026-05-31** — canonical action catalog stubs (`20260602160000`), Phase 1A entry actions, tour/enrollment alignment migrations, BOS drawer header attention — **`docs/sprints/archive/05_2026/completed/lifecycle_closeout.md`** (~84%; **`move_to_waitlist`**, `mark_won` cutover, BOS wiring deferred).
- **Completion guardrails (contextual validation)** — **~2026-05-30** — structured validation on lifecycle execute paths — **`docs/sprints/archive/05_2026/required_fields_completion_guardrails_audit.md`**.
...
3. **Tour Scheduling Phase 2 Band B+** — calendar sync, public hardening, analytics — **`docs/sprints/archive/05_2026/later-phase/tour_scheduling_phase_2.md`** (Band A closeout: **`completed/tour_scheduling_phase2_band_a_closeout.md`**).
...
- **Enrollment Packet E2E Phase 1:** **Shipped** — **wall clock 2026-05-07 (Thu) → 2026-05-13** — see **`docs/product/documents-and-forms.md`**, **`docs/sprints/archive/05_2026/enrollment_journey_packet_operations_v1.md`** (status banner).
- **Shipped (entire forms product “complete”):** TBD *(Phase 2 + vertical polish — **`docs/sprints/archive/05_2026/enrollment_packet_phase_2.md`**)*
...
- **Enrollment Packet Phase 2** — **partially implemented**: **P2-1–P2-4 review MVP shipped ~2026-05-21**. **Still open:** DCP, P2-5 BOS insight, UX hardening cards — **`docs/sprints/archive/05_2026/later-phase/enrollment_packet_phase_2.md`**, **`forms_documents_phase_2_packet_review_mvp.md`**
...
- Enrollment **`work_units.queue_definition`** alignment with **`enrollmentPipelineQueueDefinitionV1`**; **`needs_attention`** overlay + optional **`needs_attention_buckets`** metadata; department pipeline rows — see **`docs/product/crm-system.md`**, **`docs/archive/2026-06-superseded-system/workspace-system.md`**.
