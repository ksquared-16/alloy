...
| `payments/run/route.ts` | “No auth” | **Batch 3:** `getAdminContext` + asserts org-owned **`jobs`** row before proxy; **charge/ledger** org enforcement still **Python backend** (see `docs/implementation/ADMIN_API_REMEDIATION_BATCH_3.md`). |
...
- **Remediation** — **Batch 1** / **Batch 2** / **Batch 3** fixes where applied; **—** = not yet remediated in those batches (see `docs/implementation/ADMIN_API_REMEDIATION_BATCH_1.md`, `docs/implementation/ADMIN_API_REMEDIATION_BATCH_2.md`, `docs/implementation/ADMIN_API_REMEDIATION_BATCH_3.md`).
