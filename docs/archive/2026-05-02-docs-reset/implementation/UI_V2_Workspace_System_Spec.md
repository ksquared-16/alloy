...
> **Doctrine (2026-04):** Cross-cutting rules for **resolver-backed records**, **overview layout (no page builder)**, **queue vs record truth**, and **scope** are in [`docs/architecture/`](../architecture/README.md). This spec’s **block/level model** remains a useful companion; if AI or layout text here conflicts with `overview-layout-doctrine.md` / `record-rendering-system-spec.md`, **prefer the architecture folder**.
