...
> **Doctrine (2026-04):** Platform semantics for **record vs work unit vs queue vs drawer** are canonical in [`docs/architecture/workspace-work-unit-scope-doctrine.md`](../architecture/workspace-work-unit-scope-doctrine.md). This document remains the **Admin V2 UI column contract** for a specific shell implementation — useful for pixel/layout consistency, **not** the source of truth for the future RRS-backed workspace system.
