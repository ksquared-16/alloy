...
- **Sprint closeout:** Settings/record UX parity shipped **2026-05-18** — see **`docs/sprints/archive/05_2026/settings_record_ux_parity_sprint.md`** §12 for shipped vs deferred; regression manifest `web/tests/sprints/settingsRecordUxParityRegression.test.ts`.
...
3. **Queue lists:** `QueueService` builds **preview** projections (allowlisted columns, sorting, filters) for jobs/opportunities/etc. — optimized for lane triage, not full record authority. For opportunity **CRM compact** lanes, child display lines may be **enriched from `customer_members`** while remaining preview-only (see **`docs/archive/2026-06-superseded-system/workspace-system.md`**).
