...
- **Child enrollment lifecycle (May 2026):** **`opportunity_customer_members.outcome_status_key`** is **per-child enrollment lifecycle SoT** (status definitions where **`entity_type = 'opportunity_customer_members'`**). **`opportunities.status_key`** remains case-level coordination — see **`docs/product/crm-system.md`**, migrations **`20260601100000`**, **`20260601150000`**.
- **Placement candidates (waitlist grain):** **`placement_candidates`** — child × site/cohort waitlist membership rows used by QueueService v2 candidate lanes (**preview/triage**; not global waitlist truth). Related: **`opportunity_customer_members.location_id`**, **`program_room_cohort_key`** for child site/room authority — **`docs/archive/2026-06-superseded-system/entity-model.md`** § Location semantics.
