...
**Multi-code buckets:** A bucket that lists **multiple** `reason_codes` counts an inquiry **once** if **any** listed reason appears in **`reasons[]`**. The work-unit URL filter only accepts one **`attention_reason_code`** today — multi-code buckets should open the **full** Needs attention queue without that filter (product copy should say so).

**Platform default bucket catalog** includes several starter buckets (see `DEFAULT_NEEDS_ATTENTION_BUCKETS`); orgs override/extend via metadata.
