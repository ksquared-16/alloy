...
| **C5 — Editable Card Runtime** | ✅ **Built + first card migrated.** Canonical runtime (`web/lib/experience/editing`, 16 tests); Pattern A elevated; one save coordinator kept. `EditablePersonContactCard` **migrated** onto `useEditableCardRuntime` — self-managed `saving`/`savedFlash`/`saveError` removed, silent-loss-on-failure bug fixed; rotted test net repaired + new jsdom interaction lifecycle test. New cards must use the runtime. Other cards: next increments. |
