...
> **Reconciliation note (2026-07, Operational Expansion Wave 1 freeze — RFC D12).** The pipeline above is complete **as a library**, but the **fact→consumption trigger edge is a named, still-pending seam**: today the pipeline is invoked only by the `/consumption/simulate` path — no attendance/schedule/agreement fact write auto-invokes it. Closing that edge **safely** (correction/reversal-aware, so a corrected or reversed fact supersedes rather than duplicates its obligation) is **Implementation Wave 1 — D12a**, specified in [`../operational-expansion-wave1-implementation-spec.md`](../operational-expansion-wave1-implementation-spec.md); wiring the reactor to real facts (preview-first) is the later **D12b**. See the frozen [`../rfcs/operational-expansion-phase1.md`](../rfcs/operational-expansion-phase1.md) D12.
