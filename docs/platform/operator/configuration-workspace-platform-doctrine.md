...
**Top-level landing convergence (V2.2):** Organization and Locations use the same landing-page rhythm without sharing a forced data model. A compact identity/posture band leads directly to equal-height peer objects. Organization uses dense Domain Cards; Locations uses compact Location tiles carrying identity, active state, locality, Rooms, Programs offered, known capacity, readiness, highest-priority attention, and Open. Locations keeps search, inactive filtering, Add Location, and scope switching. Its lower row gives Needs Attention approximately two-thirds and Fleet Summary one-third. Detailed explanation remains inside the selected domain or Location runtime; no readiness, attention, mutation, or ownership logic moves into landing presentation.
