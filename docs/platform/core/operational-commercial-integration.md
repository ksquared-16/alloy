...
| **A1a — GL mapping bootstrap + shared JE writer** | Populate `financial_charge_templates.default_gl_mapping_key`; seed an AR account + `asset_accounts_receivable` mapping (and revenue mappings per category). Extract `writeBalancedJournalEntry(...)` from `postCashEvent`'s JE mechanics (idempotent upsert, line delete+reinsert, balance assertion, `billable_source_*` stamping). | The true blocker: today every charge's `metadata.gl_mapping_key` is **null** and **no AR mapping is seeded**, so nothing can resolve. Needs a small config/seed migration. The JE *mechanism* is reusable; `postCashEvent` itself is not (see §11c). |
| **A1b — `postEnrollmentCharge`** | Per reviewed draft charge: resolve revenue + AR accounts, write a balanced **Dr AR / Cr Revenue** JE (`source_type='enrollment_charge'`, `source_id=charge_id`), then flip `draft→posted`. Fail-closed on missing mappings. Positive revenue categories only. | The authoritative write. Operator-initiated, idempotent, gated on `review_status='reviewed'`. |
| **A1c — Finalize action / UI** | A `finalize` action on the obligations API + a Finalize/Post button on reviewed rows in `obligation-review/page.tsx`; surface `missing_mappings` blocks inline. | Connects the A0 gate to the A1b write. Law 1: operator-initiated only. |
...
**A0 is complete.** Recommended order: **A1a → A1b → A1c → B → C → D.** A1a is the critical path — without GL mappings the rest resolves nothing (§11c). All of A1 stays inside Law 1: Posting is operator-initiated (Finalize), never autonomous.

### 11c. A1 Posting Preflight (read-only verification, 2026-06-30)

A direct read of the GL substrate (`gl_accounts`, `gl_account_mappings`, `gl_journal_entries`, `gl_journal_lines`, `postCashEvent.ts`, `childcareChargeService.ts`, `billableSource.ts`, `chargeLifecycleService.ts`, `billing-financials-platform.md`) found the substrate ready but the enrollment posting **path** unwired. Findings that bind A1's design:

1. **Enrollment charge posting is NOT cash posting.** `postCashEvent` records *cash movements* (Dr cash / Cr clearing|payable). Posting a charge is *revenue recognition* — a different accounting event with a different trigger (a reviewed charge, not a completed schedule).
2. **`postCashEvent` must NOT be generalized in place.** It is coupled to schedule + job + vendor-payout-policy and to cash mapping keys. Widening it to "also do charges" would entangle two distinct events.
3. **Extract a shared JE writer instead.** `writeBalancedJournalEntry(...)` lifts the reusable mechanics (idempotent upsert by `(org_id, source_type, source_id)`, line delete+reinsert, Σdebits=Σcredits, 23505 handling). `gl_journal_lines` already carries `billable_source_type`/`billable_source_id` (P3.1) — **no table migration needed** for line stamping.
4. **Enrollment charge posting is Dr Accounts Receivable / Cr Revenue.** The charge's `metadata.gl_mapping_key` identifies the **revenue** (credit) account; a standard `asset_accounts_receivable` key supplies the **AR** (debit) counter-account.
5. **Posting requires, per charge:** a **reviewed** obligation (`review_status='reviewed'`, `eligibleForPosting`); a **draft** charge (`billable_source_type='enrollment_agreement'`, `status='draft'`); a resolvable **revenue GL mapping** (from `metadata.gl_mapping_key`); a resolvable **AR GL mapping**; and **billable-source stamping** on the JE lines (`billable_source_type/id` + `customer_id`).
6. **Missing mappings must fail closed.** If the revenue or AR mapping can't resolve, return a typed `missing_mappings` error, write **nothing**, and leave the charge in `draft` (pre-check both accounts *before* any insert — keep posting atomic; surface the block on the review surface).
7. **A1 needs a small config/seed migration** (no table DDL): populate `financial_charge_templates.default_gl_mapping_key` (today **null** on all templates) and seed an **AR** account + `asset_accounts_receivable` mapping (plus revenue mappings per category). This is the actual blocker — revenue mappings exist only in the demo seed, and **no AR mapping is seeded anywhere**.
8. **Deferred categories (NOT in A1's first pass):** `credit`, `discount`, `adjustment`, `subsidy_offset` (contra-revenue / credit-memo semantics) and `deposit` (a liability, not revenue). A1 posts **positive revenue** categories only (`tuition`, `fee`, `late_pickup`, `one_time`, `consumable_fee`).
9. **Smallest safe A1:** positive enrollment charges only; **operator-initiated Finalize**; **one charge at a time**; **idempotent** JE (by `source_type='enrollment_charge'`, `source_id=charge_id`); then flip `draft→posted` after the JE succeeds.
