...
Hooks                                                │  hands to ↓
Components                                   [C] RUNTIME HOST             (client, persistent)
                                                  one Operational Shell + one RuntimeProvider
...
- **[C] Runtime Host** — *Why:* stable chrome + a single place that holds the VM and the coordinators. *Owns:* shell persistence, the VM handoff, coordinator access (one `RuntimeProvider`). *Could merge?* It **absorbs** the 8 providers and the 60 cache modules behind one context + one cache.
...
| Session identity (auth/org/labels/tz/vertical) | **RuntimeProvider** value (one) | exposed as selector hooks, but one provider |
...
| **Providers (×8)** | **REMOVE → 1** | Collapse into the `RuntimeProvider` value; React context becomes implementation detail. |
...
**Today:** 8 nested context providers, all hydrated from server props (auth, vertical, labels, viewer-tz, operational-tz, org, operational-mode, first-paint-seed) + the drawer provider.

**Target:** **one `RuntimeProvider`** exposing a single composed value:
```
runtime = {
  session: { auth, org, vertical, labels, viewerTz, operationalTz },   // server-resolved once
  vm:      RouteVM,                                                      // the surface
  cache, save, nav, warm, reveal,                                       // coordinators
  focus:   FocusPanelController,                                        // open-state + subject
}
```
- **Can providers disappear?** Yes — the 8 become *fields* of one value. Behavior providers (operational-mode, drawer open-state) become **coordinator state**, not standalone providers.
- **Can contexts disappear?** The two timezone contexts merge to one `session` field; org/labels/vertical become `session` fields; the first-paint seed becomes a `vm` field.
- **Can Route VM replace providers?** For data, yes (timezones/org/labels/first-paint are VM/session fields). For behavior, the coordinators replace them.
- **Can React context become an implementation detail?** **Yes — this is the key shift.** Today context *is* architecture (8 layers). Target: context is how the Runtime Host distributes one value; consumers use thin selector hooks (`useSession()`, `useRouteVM()`, `useSave()`), so call sites are stable while the architecture is one provider.
...
## North star
This is the runtime Alloy deserves: **one shell, one server-composed VM per surface, one provider value, one cache, one save path, predictive warm.** The migration from today's stack to this — incremental, additive, flag-gated — is specified in [`runtime-simplification-v1.md`](../../sprints/06_2026/runtime-simplification-v1.md). Design only; no code until reviewed.
