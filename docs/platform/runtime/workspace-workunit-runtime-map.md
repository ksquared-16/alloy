...
| `WS.PROCESS_TILE` | `WS.PROCESS_TILE` | Single process entry card | `ProcessNavTile` (sole renderer — `EnrollmentOperationalSurfaceTile` removed) | `OperatorLifecycleLandingCard` | (inherited from grid) | Lifecycle catalog + WU queue definition | `WS.PROCESS_TILE_WORK_VIEWS` | Canonical |
...
| `WU.HEADER` | `WU.HEADER` | Work unit header banner | `WorkUnitCommandSurface` (sole owner — legacy brief div label removed) | `WorkUnitWorkspaceModel.focusLabel` + KPIs | Bootstrap payload | None | `WU.HEADER_TITLE`, `WU.HEADER_CALCULATIONS` | Canonical |
...
| `WU.FOCUS_PANEL` | `WU.FOCUS_PANEL` | Focus Panel / record detail drawer | `AdminDrawerContext` → drawer surface | Opportunity / record VM | `AdminDrawerContext.openDrawer()` → record bootstrap | Record layout config | None | **Deferred** — focus panel is context-managed and not static in the component tree; `data-alloy-section` label cannot be placed at build time. Label pending context surface refactor. |
...
| `EnrollmentOperationalSurfaceTile` component | **Deleted** | All tiles render via `ProcessNavTile` — no product-hardcoded renderer |
| `ensureEnrollmentOperationalSurfaceCard()` adapter | **Deleted** | No adapter needed — grid maps `OperatorLifecycleLandingCard` directly |
| `isEnrollmentLifecycleCard()` branch in grid | **Deleted** | Single render path; config drives content |
| `LegacyProcessNavTile` function name | **Renamed → `ProcessNavTile`** | Canonical tile renderer no longer named Legacy |
| `WU.HEADER` / `WU.HEADER_TITLE` on brief div | **Removed** | Sole owner is `WorkUnitCommandSurface`; legacy brief path is unlabeled |
