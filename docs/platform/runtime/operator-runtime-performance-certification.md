...
39. **ONE RESOLVER IS NECESSARY, NOT SUFFICIENT — THE INPUTS MUST MATCH TOO.** Two call sites sharing a resolver still disagree when they feed it different facts. The waitlist projection resolves a cohort from the candidate's stored key/label plus OCM context; the ensure path has only process-instance facts, so the same owner returned `infant_0_18_months` for one and `unknown_program_room` for the other. Corollary, and the expensive half: **a migration that changes an identity key changes it for every row at once**, so any write that migration performs BEYOND identity is a mass mutation by definition — here it rewrote 14 of 17 stored cohorts on a single page load and re-sectioned a live waitlist. A key migration must write the key and nothing else.
