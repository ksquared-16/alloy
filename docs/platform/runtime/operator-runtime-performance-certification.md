...
34. **An identity's WRITE target and its READ source must be the same owner.** Child identity writes to `persons` when the child is person-linked and never to `customer_members`; every operator surface — Children card, drawer VM, durable-child subject, Records — reads `customer_members`. So the first edit diverges the two copies, the divergence is invisible wherever an operator would look, and it is **unrepairable through the product**: the editor baselines on the member copy, so typing the correct value is a no-op and writes nothing. That is not a stale cache — it is a second writable truth, and it is how `perf-probe-1787311039569` stood in a live projection for weeks while every surface showed the right name. Do not fix this by mirroring one field into the other (that keeps two truths and adds a sync); make the surfaces derive from the owner the write actually targets.
