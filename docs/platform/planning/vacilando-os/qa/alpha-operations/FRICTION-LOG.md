...
### [VALIDATED] Director knows what is happening but does not express it when the operator becomes uncertain — Operational Presence
- when: across the engineering lifecycle, most sharply right after **Start** — driving the Access & Roles inventory mission end-to-end, the work read **"running" for ~30s with no sign the engine had engaged** before the worker actually attached and progress began. Also observed: Director says it has questions but does not immediately ask; review appears abruptly rather than arriving; long normal execution feels like inactivity.
- what happened: the engineering **state was correct**, but the product went quiet at exactly the moments the operator becomes uncertain. The operator was left to interpret dead air ("is it stuck? did anything change? who has the next move?"). The product effectively *disappeared* — and an engineering director never disappears.
- escape required: none, but during the launch dead-air the operator's instinct is to open the provider window / `ps` to confirm the engine actually started — the exact "leave Vacilando to check if it's alive" failure the OS exists to remove.
- recurrence: reproducible on every mission Start (the launch gap) and observed repeatedly across stages in one session — this is a pervasive product property, not a one-off.
- hypothesis: **Operational Presence was missing.** Presence was a single flat line (`directorSays`) keyed only on coarse status: `starting`/`running` both said "executing now," so the launch window (engine not yet attached) was indistinguishable from real execution. There was no *Launching* state, no event on meaningful transitions, and no honest voice for Understanding / Reviewing / Closed. The information existed in durable state (`current_phase`, `last_activity_at`); it simply was not voiced.
- **RESOLUTION (this sprint):** realized Operational Presence as a pure, event-driven projection (`presence.mjs`), the single Director voice for every stage — derived from the same durable state, silent when nothing meaningful changed (no manufactured reassurance). The executor now stamps its **real** launch steps as honest phases (*preparing worker → verifying environment → attaching engine → dispatching work*), and presence distinguishes **Launching** (dispatched, no activity yet) from **Executing** (worker has produced activity), rendering the launch sequence so the operator watches the worker come online instead of interpreting dead air. Executing speaks coarse, event-driven phases (*exploring the code / writing the deliverable / verifying*); Understanding leads with the question; Reviewing "arrives"; Closed explicitly releases. No new runtime, no dashboard, no chatter. Proven live + 6 focused presence tests. This is the **first enduring Alpha observation** to complete the loop: observed friction (dead air during a real run) → hypothesis (missing presence) → shipped product improvement (event-driven presence).
