...
- **[scheduling-focus-panel-composition.md](./scheduling-focus-panel-composition.md)** — *(final)* the Focus Panel cleanly separated into **Identity · Work · Commands**. The Summary card is pure identity (room · pattern · effective dates · status — no calculations, health, or recommendations); operational work ("over ratio Thursday", "needs a room") moves to a **Work card** (Current Work / Needs Attention); siblings become quiet **Household** context; effective dating is made obvious; commands stay configured. This closes product discovery.
...
- **[mockups/scheduling-focus-panel-final.html](./mockups/scheduling-focus-panel-final.html)** — *(final)* the Identity / Work / Commands separation: pure identity cards (six states), identity + a distinct Work card, Household context, opened-from-roster/enrollment, and the configured commands (change · change room · end · temporary move).
