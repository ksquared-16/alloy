...
- **[mockups/scheduling-projection.html](./mockups/scheduling-projection.html)** — *(final)* the same Scheduling card projecting a **household** vs a **child**, the lifecycle (Current · Upcoming · Temporary · History), **multiple assignments** in one schedule (split week), rate changes, and the pattern editor as a focused command.
