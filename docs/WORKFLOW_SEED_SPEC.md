...
| 5   | Job action: assign_vendor → job status assigned      | v_org_id | true    | job_action                 | job         | field action eq "assign_vendor"                         | update_entity job: id_path job.id, patch { job_status_id: "{{assigned_job_status_id}}" }. (Route does not pass assigned_job_status_id; use your org's job_status id for key=assigned or add to route.)                                                                                                                                                                                                                                                                                                                        |
| 6   | Job action: mark_completed → job completed           | v_org_id | true    | job_action                 | job         | field action eq "mark_completed"                        | update_entity job: id_path job.id, patch { job_status_id: "{{completed_job_status_id}}", completed_at: "{{occurred_at}}" }.                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Placeholders in payload (templates): `{{booked_stage_id}}`, `{{quote_started_stage_id}}`, `{{scheduled_job_status_id}}`, `{{assigned_job_status_id}}`, `{{completed_job_status_id}}` — either pass from routes or replace in workflow payload with actual UUIDs for your org after seed.
...
- `{{scheduled_job_status_id}}` — add to book-v2/confirm payload (lookup job_status id for key=scheduled) or set in workflow payload to your org's UUID.
- `{{assigned_job_status_id}}` — add to admin job PATCH or set in workflow to your job_status id for key=assigned.
- `{{completed_job_status_id}}` — add to admin job PATCH or set in workflow to your job_status id for key=completed.
...
2. **booking_confirmed**
   ...
   - Confirm `workflow_runs` has a completed run and that the opportunity's `pipeline_stage_id` and `status`, and the job's `job_status_id`, were updated as expected.
