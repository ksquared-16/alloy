...
1. **Slice B — work-unit queue layout-doc reveal coordination** (with runtime test suite) — removes the visible `/work-units` flash.
2. **Slice C — `SettingsSurfaceViewModel`** over a hub readiness adapter; converge one settings family; prove single-owner.
