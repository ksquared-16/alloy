...
**Status:** **Task Assist V1.1 complete** (Cards **0–9**, including **9b** entity search + **9c** intent routing). **Interaction Layer V1** (Cards **1–7** in **`docs/sprints/05_2026/agent_interaction_layer_v1.md`**) superseded Card 9 **UX** — universal bottom command bar with thread + action cards; **backend routes unchanged**. **Agent #2 (Task Assist)** operator UI lives in the AdminV2 bottom **`AICommandSurfaceShell`** — not a right rail or drawer-embedded panel.  
...
- **Command bar = primary assistant surface (Card 9 + Interaction Layer V1)** — **`AICommandSurfaceShell`** at the bottom of **`AdminV2Shell`** is the always-visible assistant home. **Single input**, **no visible mode tabs**; **`routeCommandSurface`** decides Task Assist vs job layout vs Workflow Assist notice. **Thread + action cards** show user turns, search candidates, target confirm, and collapsed **`TaskAssistOpportunityWorkspace`** behind **Review & approve**. **`GlobalAssistantProvider`** holds **`currentContext`** and **`focusCommandBar`**; **`commandSurfaceMode`** remains internal only (drawer launcher compat). Gated by **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. Card 8 right **`GlobalAssistantShell`** removed; Card 6 removed header **Assistant** link.
...
- **Workflow Assist notice** — Workflow/automation NL (e.g. “when forms complete…”) shows **Workflow Assist — coming next** in the thread; no Task Assist proceed, no workflow config.
...
- **Opportunity context, not drawer-owned UI** — Drawer communications tab: **`TaskAssistOpportunityLauncher`** + **`CommunicationsDrawerSection`** (threads) only; Task Assist actions in command bar thread + cards.
- **Distinct from AI log** — bottom bar = universal assistant; **AI log** = apply history.
