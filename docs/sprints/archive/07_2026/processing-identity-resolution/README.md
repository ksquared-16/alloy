...
**E1 status: implemented locally — awaiting full sprint validation and promotion** — Superseded direct-write intake paths retired or gated: `applyFormIntakeSafe` throws unless `__legacyDirectWriteReplay` (historical tests only); active routes delegate to canonical adapters. Static boundary tests: `web/tests/processing/processingIdentityE1Boundaries.test.ts`. Contacts global-uniqueness cleanup deferred — D4/D5 cutovers eliminate active contacts writes on these intake paths; remaining callers documented for later phase.
...
| E1 | see git log | `processingIdentityE1Boundaries.test.ts` | `applyFormIntakeSafe` gated behind `__legacyDirectWriteReplay`; static import-boundary tests; contacts uniqueness cleanup deferred |
