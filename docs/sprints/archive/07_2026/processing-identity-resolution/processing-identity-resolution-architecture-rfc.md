...
**Status:** **Implemented locally · Locally certified · Reconciled onto latest `origin/staging` · Awaiting PR merge to staging · Not deployed.** Product-owner decisions (new-record thresholds, 180-day reopen policy, retention classes) are incorporated. **Baseline (design):** `origin/staging` @ `65afc8527`; **certified branch:** `claude/proc-identity-lib-normalization` (includes identity-review eligibility gate). See [open-decisions](processing-identity-resolution-open-decisions.md). Evidence: **[C]** confirmed, **[P]** design-time proposal, **[D]** doctrine.

**Implementation reconciliation.** V1 is implemented through E1, including the certified identity-review gate. Manual Create Lead and public lead-capture forms are authoritative Processing adapters; neither has a legacy direct-write fallback. C1 shadow comparison code is retained only as audit tooling. D0–D5 have no runtime feature flag. Identity mutation remains human-authoritative through an immutable plan, exact approval binding, eligibility fail-closed checks, and explicit execution.
