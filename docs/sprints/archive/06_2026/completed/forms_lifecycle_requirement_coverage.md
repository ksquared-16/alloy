...
## Card 5 — Runtime Server Validation (implemented)

### Doctrine

- **Card 4** checks whether the **form schema** captures required fields (operator readiness).
- **Card 5** checks whether the **submitted payload** contains required values before CRM intake.
- Client-side gating improves UX; server-side validation protects data integrity.

### Hook location

`POST /api/public/forms/[token]/submissions/[submissionId]/submit`

After `buildFormIntakeMetaFromPayload` succeeds and **before** `applyFormIntakeSafe`:

1. `validatePublicSubmissionLifecycleRequirements`
2. On block → persist draft payload meta, return `400 LIFECYCLE_VALIDATION_BLOCKED`
3. On pass/skip → existing intake path unchanged

### Modules

| File | Role |
|------|------|
| `evaluateFormsLifecycleFieldCoverage.ts` | Optional `submittedValues` on mapper — Card 5 runtime path |
| `evaluateSubmittedFormsLifecycleFieldCoverage` | Schema + submitted values evaluation |
| `validatePublicSubmissionLifecycleRequirements.ts` | Load usage/contract, evaluate, build public + admin meta |
| `buildLifecycleValidationBlockedMeta` | `lifecycle_validation_blocked`, missing labels, no auto-op |

### When validation runs

Requires all of:

- `linkRequiresLeadCapture(link metadata)`
- `auto_create_opportunity` on link
- `metadata.lifecycle_usage_v1` on form definition
- Record-creating intent (`enrollment_lead`, `waitlist`)

**Skipped (legacy):** no lifecycle usage configured — demo enrollment lead proof path unchanged.

### Blocked submission behavior

- **Public:** `400` with operator-friendly message, e.g. “This form is missing required information: Guardian phone or email.”
- **Draft payload meta:** `lifecycle_validation_blocked`, `missing_required_fields`, `lifecycle_usage_v1`, `intake_resolution_path: lifecycle_validation_blocked`
- **No CRM side effects:** no `applyFormIntakeSafe`, no `opportunity_id`, no `intake_auto_operationalized`

### Rules (same as Cards 1–2)

- Required missing values → block
- Recommended missing → allow
- OR constraints (`at_least_one`) honored on submitted values
- Custom/unmapped fields cannot satisfy unless mapped
- Non-record intents / no lifecycle usage → skip
...
npx tsx scripts/qaEnrollmentLeadOpportunityProof.ts
npx tsx scripts/qaEnrollmentIntakeLifecycleCoherence.ts
```

### Remaining gaps (Card 6)

| Item | Gap |
|------|-----|
| **Card 6 — QA + docs closeout** | End-to-end operator QA with lifecycle usage configured on Website Inquiry, enrollment hub alignment, sprint closeout |
| **Optional follow-up** | Server-side block on `POST /public-links` creation (Card 4 is client-only today) |

**Card 6 readiness:** Cards 1–5 complete; runtime + share gating + QA proof green.

---

## Card 6 — QA + Sprint Closeout (completed)

### Sprint outcome

**Lifecycle Builder defines the field contract. Forms consume it. Forms validate readiness. Runtime enforces it.**

| Card | Deliverable | Status |
|------|-------------|--------|
| 0 | Audit + sprint plan | ✅ |
| 1 | `resolveFormsLifecycleRequirementContract` | ✅ |
| 2 | `evaluateFormsLifecycleFieldCoverage` (field-source-first) | ✅ |
| 3 | Lifecycle Usage UI + coverage API | ✅ |
| 4 | Share/readiness gating (client) | ✅ |
| 5 | Public submit runtime validation | ✅ |
| 6 | QA closeout + doc move | ✅ |

### QA paths verified

| # | Path | Result |
|---|------|--------|
| 1 | Clean Website Inquiry — schema + runtime ready, share allowed, lead created | ✅ Logic + live proof |
| 2 | Missing required — share blocked (schema), runtime 400 (values), no CRM records | ✅ |
| 3 | Recommended-only gaps — warn, share + runtime allowed | ✅ |
| 4 | Duplicate/ambiguous lead — review required, not clean-created | ✅ Existing classification preserved |
| 5 | No `lifecycle_usage_v1` — legacy skip, no unexpected blocks | ✅ Demo enrollment lead proof |
| 6 | Location-specific links — distinct from general; routing via link metadata | ✅ Presentation tests |

### Closeout automation

- **Vitest:** `tests/forms/formsLifecycleRequirementCoverageCloseout.test.ts` (7 path tests)
- **Gate script:** `web/scripts/qaFormsLifecycleRequirementCoverageCloseout.ts`

### Tests / scripts run (Card 6 sign-off)
...
cd web && npm run test -- \
  ...
  tests/forms/isFormLifecycleReadyForRecordCreation.test.ts \
  tests/forms/validatePublicSubmissionLifecycleRequirements.test.ts \
  tests/forms/formsLifecycleRequirementCoverageCloseout.test.ts
cd web && npx tsx scripts/qaFormsLifecycleRequirementCoverageCloseout.ts
```

**Results (2026-06-02):** 58 vitest tests across 9 files — pass. `qaEnrollmentLeadOpportunityProof.ts` — pass. `qaEnrollmentIntakeLifecycleCoherence.ts` — pass.

### Known limitations (future phases)

- **Link creation enforcement** — Card 4 gating is **client-side**; `POST /public-links` is not server-blocked unless added later.
- **Enrollment Process hub** — `enrollmentProcessFormCoverage.ts` still uses label/id heuristics; not unified with Form Detail coverage panel.
- **Complex constraints** — OR support matches lifecycle engine today (e.g. email-or-phone); broader AND/OR trees are future.
- **Location routing** — Campus routing is **link metadata** (`default_location_id`), not form schema.
- **Recommended fields** — Warn only; never block share or runtime.
- **Non-record forms** — Document-only / existing-family / no lifecycle usage — not lifecycle-blocked.
- **Delete/archive test lead UI** — Future operator tooling.

### Future phase items

1. Server-side block on record-creating public link mint when lifecycle coverage fails.
2. Unify Enrollment Process hub coverage with Form Detail lifecycle panel.
3. Operator UI to delete/archive test leads from lifecycle QA.
4. Broader constraint language in Forms coverage mapper when lifecycle engine expands.
