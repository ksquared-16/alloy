...
- **Orchestrator + specialists (AdminV2 command bar):** Bottom **`AICommandSurfaceShell`** is the **Orchestrator** — routes to **Task Assist** (comms, scheduled sends, operational tasks), **Workflow Assist** (read/explain/propose/apply — admin-gated), and **Config/Layout Assist** (configuration proposals — **partial** apply). **Does not execute** side effects itself. Drawer **communications** tab: **`TaskAssistOpportunityLauncher`** + threads. Flag: **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. See **`docs/product/ai-system.md`**. **`metadata.next_follow_up_at`** auto-sync from tasks is **not** in V1.1.
