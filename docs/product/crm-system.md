...
- **Task Assist V1.1 (drawer):** Operators can save AI draft proposals (**`task_assist_proposals`**), schedule one-time outbound messages (**`communication_scheduled_sends`** + **`process-due`** worker), and create opportunity-scoped **`operational_tasks`** (**`source: task_assist`**) from **`TaskAssistV1OpportunityPanel`** when **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`** is on — see **`docs/sprints/05_2026/task_assist_v1_1.md`**. Automatic sync of **`metadata.next_follow_up_at`** from tasks is **not** part of V1.1.
