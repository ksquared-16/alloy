...
- **Task Assist V1.1 (global assistant, opportunity context):** Operators save AI draft proposals (**`task_assist_proposals`**), schedule sends (**`communication_scheduled_sends`** + **`process-due`**), and create opportunity-scoped **`operational_tasks`** (**`source: task_assist`**) from the **global AdminV2 Assistant** shell (**`TaskAssistOpportunityWorkspace`**). The opportunity drawer **communications** tab provides **`TaskAssistOpportunityLauncher`** only (context + open) when **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`** is on — see **`docs/sprints/05_2026/task_assist_v1_1.md`**. Automatic sync of **`metadata.next_follow_up_at`** from tasks is **not** part of V1.1.
