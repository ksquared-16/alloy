...
- **Waitlist priority fact truth + child scope (Card 0–1, May 2026):** Per-child site/cohort on **`opportunity_customer_members`** (`location_id`, `program_room_cohort_key`); employee priority from **`persons.is_employee`**; V2 evaluator resolves sibling enrolled / sister-center from OCM lifecycle + site (not opportunity metadata when household context is loaded). **`placement_candidates.site_id`** prefers OCM with opportunity fallback + diagnostics. Sprint: **`docs/sprints/05_2026/waitlist_priority_fact_truth_child_scope.md`**.
