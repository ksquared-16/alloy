...
- **Task Assist V1.1 / Agent 2 (AdminV2 command bar):** Primary surface is bottom **`AICommandSurfaceShell`** — **single universal assistant input** (no visible modes), **`routeCommandSurface`** for Task Assist vs job layout vs Workflow Assist notice, **thread + action cards**, slot-based entity/goal extract, ambient context from drawer/page, or **`GET /api/admin/ai/task-assist/entity-search`** for named families/opportunities, then **Confirm target**, then collapsed **`TaskAssistOpportunityWorkspace`** (draft proposals, send-now, **`task_assist_proposals`**, **`communication_scheduled_sends`**, **`operational_tasks`**). **Never auto-send**, **no bulk**, **no workflow config** in Task Assist. Drawer **communications** tab: **`TaskAssistOpportunityLauncher`** + threads only. Flag: **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. See **`docs/sprints/05_2026/task_assist_v1_1.md`**, **`docs/sprints/05_2026/agent_interaction_layer_v1.md`**. **`metadata.next_follow_up_at`** auto-sync from tasks is **not** in V1.1.
