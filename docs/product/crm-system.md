...
- **Inquiry children (drawer):** **`OpportunityInquiryChildrenSection`** — one **horizontal row per child** (identity on **`customer_members`**, inquiry fields on **`opportunity_customer_members`**). Matches work-unit CRM compact child lines via **`inquiryChildrenHydration.ts`**. See **`docs/sprints/archive/05_2026/linked_record_field_editing_v1.md`** § V1b and **`record-system.md`**.
...
- **Drawer:** **Operational Attention** chrome is a single header strip (`OperationalAttentionHeaderStrip`, `variant="chrome"`) from **`_operational_attention`** on entity GET — not a second panel in the inquiry summary. **Review Assist** (deterministic operational read + urgency) lives in the inquiry summary right column; **Work with BOS** (`BosDrawerAssistCta`) hands off to Task Assist / Workflow Assist via **`bosAssistHandoffRouting`** (deterministic intent — not NL misroute). Below tabs, **`OpportunityOperationalCompactStrip`** may surface tasks/scheduled sends. Operators review/approve drafts in the command surface (`AICommandSurfaceShell`); **no auto-send**. Closeout: **`docs/sprints/archive/06_2026/completed/bos_assist_routing_communication_drafting_closeout.md`**.
...
- **Placement priority V1 (shipped, opt-in):** When **`work_units.metadata.placement_priority_v1.enabled`** is true for a lane in **`queue_keys_enabled`**, **`QueueService`** attaches **`_placement_priority`** previews and may reorder within the evaluation cap (`web/lib/orchestration/placement/applyPlacementToOpportunityQueueRows.ts`). Operators configure **Waitlist Ranking Policy** at **`/adminV2/settings/placement-priority`** (priority factors, ranking mode, PATCH work-unit metadata) — **`docs/sprints/archive/05_2026/waitlist_ranking_policy_settings_v2.md`**. Presets live under **`web/lib/orchestration/placement/presets/`** (e.g. **`childcare_enrollment_waitlist_v1`**). **Default:** placement **off** — omitting metadata behaves like pre-V1 queues.
- **Waitlist orchestration Phase 2 (Cards 0–7, pilot-ready):** Schema + backfill + **`QueueService` V2** (`engine_version: "v2"`) fans out waitlist lanes to **candidate rows** (`_placement_waitlist_row`, one child × cohort per list row). Manual order adjustments (Card 5) with activity events. **Forecast hooks (Card 6):** optional `placement_candidates.metadata.placement_forecast_v1` → evaluator fact bag + at most one subtle queue hint — **no capacity engine**, **no ordering impact by default**. Admin **Waitlist Ranking Policy** settings (`engine version`, ranking mode / **`shadow_mode`**, profile in Advanced) at **`/adminV2/settings/placement-priority`**. V1 fallback when v2 off or missing candidates. **Pilot playbook:** **`docs/sprints/archive/05_2026/waitlist_orchestration_phase2_pilot_playbook.md`**.
- **Waitlist priority fact truth + child scope (Cards 0–5 — May 2026, closed):** Per-child site/cohort on **`opportunity_customer_members`** (drawer + **public form intake** when `intake_field_paths` map child site/cohort); employee priority from **`persons.is_employee`**; V2 record-sourced household facts; candidate repair + QA scripts. **`shadow_mode: false` deferred** until pilot checklist in sprint doc passes. Sprint: **`docs/sprints/archive/05_2026/waitlist_priority_fact_truth_child_scope.md`**. **Demo readiness (May 2026):** `waitlist_demo_v1` cleanup/reseed + walkthrough QA — **`docs/sprints/archive/05_2026/waitlist_demo_readiness_final_pass.md`**. **Ranking validation + position controls (May 2026):** runtime **`Preview position n/N` / `Position n/N`** on V2 candidate rows, exact-position manual adjustment, config-aware QA — **`docs/sprints/archive/05_2026/waitlist_ranking_validation_position_controls.md`**.
- **Legacy preview fields:** **`scoped_waitlist_position`** / compact labels may still appear — **preview-only**; authoritative promotion remains entity GET + status transitions, not queue rows.
