...
- **Waitlist priority fact truth + child scope (Cards 0–5 — May 2026, closed):** Per-child site/cohort on **`opportunity_customer_members`** (drawer + **public form intake** when `intake_field_paths` map child site/cohort); employee priority from **`persons.is_employee`**; V2 record-sourced household facts; candidate repair + QA scripts. **`shadow_mode: false` deferred** until pilot checklist in sprint doc passes. Sprint: **`docs/sprints/05_2026/waitlist_priority_fact_truth_child_scope.md`**. **Demo readiness (May 2026):** `waitlist_demo_v1` cleanup/reseed + walkthrough QA — **`docs/sprints/05_2026/waitlist_demo_readiness_final_pass.md`**.
