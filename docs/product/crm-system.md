...
- **Task Assist V1.1 / Agent 2 (AdminV2 command bar):** Primary surface is bottom **`AICommandSurfaceShell`** Task Assist mode — ambient context from drawer/page, or **`GET /api/admin/ai/task-assist/entity-search`** for named families/opportunities, then **Confirm target**, then **`TaskAssistOpportunityWorkspace`** (draft proposals, send-now, **`task_assist_proposals`**, **`communication_scheduled_sends`**, **`operational_tasks`**). Deterministic intent routing prefills channel/instruction/schedule/reminder; workflow-like NL is blocked. **Never auto-send.** Drawer **communications** tab: **`TaskAssistOpportunityLauncher`** + threads only. Flag: **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. See **`docs/sprints/05_2026/task_assist_v1_1.md`**. **`metadata.next_follow_up_at`** auto-sync from tasks is **not** in V1.1.
