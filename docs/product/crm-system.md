...
- **Placement priority V1 (shipped, opt-in):** When **`work_units.metadata.placement_priority_v1.enabled`** is true for a lane in **`queue_keys_enabled`**, **`QueueService`** attaches **`_placement_priority`** previews and may reorder within the evaluation cap (`web/lib/orchestration/placement/applyPlacementToOpportunityQueueRows.ts`). Operators configure presets at **`/adminV2/settings/placement-priority`** (PATCH work-unit metadata). Presets live under **`web/lib/orchestration/placement/presets/`** (e.g. **`childcare_enrollment_waitlist_v1`**). **Default:** placement **off** — omitting metadata behaves like pre-V1 queues.
- **Waitlist orchestration Phase 2 (Cards 0–7, pilot-ready):** Schema + backfill + **`QueueService` V2** (`engine_version: "v2"`) fans out waitlist lanes to **candidate rows** (`_placement_waitlist_row`, one child × cohort per list row). Manual order adjustments (Card 5) with activity events. **Forecast hooks (Card 6):** optional `placement_candidates.metadata.placement_forecast_v1` → evaluator fact bag + at most one subtle queue hint — **no capacity engine**, **no ordering impact by default**. Admin settings expose engine version, shadow mode, and profile at **`/adminV2/settings/placement-priority`**. V1 fallback when v2 off or missing candidates. **Pilot playbook:** **`docs/sprints/05_2026/waitlist_orchestration_phase2_pilot_playbook.md`**.
