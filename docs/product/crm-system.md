...
- **Orchestrator + Task Assist (AdminV2 command bar):** Bottom **`AICommandSurfaceShell`** is the **Orchestrator Agent** — single input, **`routeCommandSurface`**, thread + clarification/candidates; **does not execute** sends/tasks/workflows itself. **Task Assist (Agent #2)** is one routed destination for one-off comms/reminders/schedules: entity-search, confirm target, then **`TaskAssistOpportunityWorkspace`** action cards (**`task_assist_proposals`**, **`communication_scheduled_sends`**, **`operational_tasks`**). **Workflow Assist** phrases → notice only (no config). Drawer **communications** tab: **`TaskAssistOpportunityLauncher`** + threads. Flag: **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. See **`docs/product/ai-system.md`**, **`agent_interaction_layer_v1.md`**, **`task_assist_v1_1.md`**. **`metadata.next_follow_up_at`** auto-sync from tasks is **not** in V1.1.
