...
- **Drawer:** **Operational Attention** chrome is a single header strip (`OperationalAttentionHeaderStrip`, `variant="chrome"`) from **`_operational_attention`** on entity GET — not a second panel in the inquiry summary. Below tabs, **`OpportunityOperationalCompactStrip`** surfaces operational tasks/scheduled sends and a **BOS handoff** card (**Continue in Orchestrator**) that seeds **`GlobalAssistantContext`** and focuses the bottom **command bar** (`AICommandSurfaceShell`) — operators review/approve there; no browser prompts for workflow edits in the Orchestrator thread.
