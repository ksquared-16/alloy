...
**PKG-02 (conversation core schema, branch `communications-v2`):** Additive migration `20260611120000_comms_v2_conversation_core.sql` extends `communication_threads` with assignment/SLA/attention columns (`assigned_user_id`, `assigned_team_id`, `assignment_state` [unassigned|assigned, checked], `attention_state`, `first_response_at`, `sla_due_at`, `sla_state`, `last_read_at`) and adds two append-only audit tables (`conversation_assignment_events`, `sla_events`) with org-member-SELECT / service_role-write RLS mirroring the V1 foundation, plus operational indexes. Additive only — no UI, no provider, no send behavior, no destructive DDL. Service wiring lands in PKG-10.

**PKG-03 (delivery events + read receipts, branch `communications-v2`):** Additive migration `20260611130000_comms_v2_delivery_events_receipts.sql` adds receipt timestamps (`opened_at`, `clicked_at`, `replied_at`) to `communication_messages` and a provider-NEUTRAL append-only `communication_delivery_events` table (`event_type` vocabulary in `web/lib/communications/v2/deliveryEvents.ts`; `provider` is an opaque string — no provider-specific columns), with org-member-SELECT / service_role-write RLS and indexes. Adds a pure, provider-neutral `deliveryReceipts` adapter (`receiptStateFromMessage`) that composes with — does not replace — the existing `deliveryStateAdapter`. Additive only; no UI, no send behavior, no provider implementation. Webhook/worker ingestion wiring lands in PKG-07/PKG-16.

**PKG-04 (recipients + per-person preferences + audit, branch `communications-v2`):** Additive migration `20260611140000_comms_v2_preferences_recipients.sql` adds `communication_message_recipients` (multi-recipient fan-out, person-first, role to/cc/bcc), `communication_preferences` (per-PERSON consent — one row per person+category across the six freeze categories; vocab in `web/lib/communications/v2/preferences.ts`), and immutable append-only `communication_preference_events` (from/to state + source/method/actor audit), all with org-member-SELECT / service_role-write RLS and indexes. Pure taxonomy classifiers (`isMarketingCategory`, `preferenceCategoryChannel`) only — **no send-time enforcement** (PKG-08), no data backfill, no UI, no provider. The one-time backfill from the existing per-person consent field is deferred to PKG-08.
