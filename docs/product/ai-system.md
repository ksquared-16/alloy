...
- **Workflow Assist V1 (Agent 3 — Cards 4–5 + Explain v0):** **`POST /api/admin/ai/workflow-assist/propose`** returns deterministic **`WorkflowAssistSuggestionV1`** (no `workflows` writes). **`GET /api/admin/ai/workflow-assist/explain`** — read-only deterministic checklist from **`workflow_events`**, **`workflows`**, **`workflow_runs`**, **`workflow_action_runs`** (`requireAdminOrOps`; no AI policy feature). **`GET /api/admin/ai/workflow-assist/capabilities`** gates propose/apply CTAs (**`admin`** only). **`POST …/apply`** — **`requireAdmin`**; **`executeWorkflowAssistApply`**. **Org policy:** **`workflow_assist_draft`** for propose/apply only. Tests: **`web/tests/agent/workflowAssist/**`**. Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`** §12.
