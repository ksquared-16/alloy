...
- **Workflow Assist V1 (Agent 3 — Cards 4–5 + Explain v1 + NL create + product loop + AI draft enrichment):** **`POST /api/admin/ai/workflow-assist/propose`** / **`POST …/apply`** (admin-gated). **`GET /api/admin/ai/workflow-assist/explain`** — Explain v1 (`explain_engine: 1`). NL **create** routes to propose with **`workflows.metadata.scope`** from workspace route context (§15). **AI draft enrichment V1** (§17): stub advisory enrichment → deterministic normalization → compact **operator review card** (§18–19); tour reminder preview uses documented merge paths `{{opportunity.metadata.tour_date}}` / `{{opportunity.metadata.tour_time}}` plus **`reminder_intent_v1`** metadata (unresolved mappings for operator confirmation) — not `{{contact_name}}`. **Command-surface card navigation** uses shared `CommandSurfaceCardLink` + `stopPropagation` so **Open Automations** and Config review CTAs work inside the footer thread. Apply still writes **disabled** workflows only (log scaffold; no send/schedule/auto-enable). Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`** §12–19.
