...
- **Workflow Assist V1 (Agent 3 — Cards 4–5):** **`POST /api/admin/ai/workflow-assist/propose`** returns deterministic **`WorkflowAssistSuggestionV1`** (no `workflows` writes). **`GET /api/admin/ai/workflow-assist/capabilities`** returns **`can_propose_and_apply_workflow_assist`** (true only when compatibility **`role`** is **`admin`**, matching propose/apply gates) for Orchestrator UX — ops see read-only copy instead of propose/apply CTAs. **`POST /api/admin/ai/workflow-assist/apply`** applies after explicit UI confirm; **`requireAdmin`** only; **`executeWorkflowAssistApply`** mirrors admin workflow create/update field rules (creates always **`enabled: false`**). **Org policy:** **`workflow_assist_draft`** in **`allowed_features`** with same stub/OpenAI policy branches as Task Assist propose (stub still requires **`AI_ENRICHMENT_STUB_ENABLED`**). Tests: **`web/tests/agent/workflowAssist/**`** (including **`workflowAssistEditProposeApplyRoute.test.ts`**, **`workflowAssistCapabilitiesRoute.test.ts`**). Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`**.
- **Orchestrator + Task Assist V1.1 (Interaction Layer + Agent #2):** **Orchestrator** = bottom **`AICommandSurfaceShell`** — single input, **`routeCommandSurface`**, thread + clarification/candidate turns, routes to specialists; **does not execute** sends/schedules/tasks/workflows itself. **Task Assist** = one routed destination for comms/reminder/schedule intents — same admin routes as V1 plus **`POST`/`GET /api/admin/ai/task-assist/proposals`**, approve/reject, **`GET /api/admin/ai/task-assist/entity-search`**, **`/api/admin/communication-scheduled-sends`** (+ **`process-due`**), **`/api/admin/operational-tasks`**; **`TaskAssistOpportunityWorkspace`** inside action cards with operator approval. **Workflow Assist** = second specialist: **`workflow_assist`** route → read-only summary/failed-run/enrollment/explain cards; **`GET /api/admin/ai/workflow-assist/capabilities`** gates propose/apply CTAs to **admin** compatibility role; structured **`POST /api/admin/ai/workflow-assist/propose`** (admin-only, org **`workflow_assist_draft`** policy); **`POST …/workflow-assist/apply`** is **`requireAdmin`** only and reuses **`executeWorkflowAssistApply`**. Slot extract: **`commandSurfaceSlotExtract`**; session thread in **`GlobalAssistantProvider`**. Gated by **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. Sprints: **`agent_interaction_layer_v1.md`**, **`task_assist_v1_1.md`**, **`workflow_assist_v1.md`** (Cards 4–5 + stabilization).
...
| **Capabilities hint (Workflow Assist)** | — | Portal session (`getAdminContextCached` compatibility **`role`**) | `GET /api/admin/ai/workflow-assist/capabilities` |
