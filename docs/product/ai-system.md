...
- **Task Assist V1.1 (durable layer + global AdminV2 shell):** Same admin routes as above plus **`POST`/`GET /api/admin/ai/task-assist/proposals`**, approve/reject, **`/api/admin/communication-scheduled-sends`** (+ **`process-due`**), **`/api/admin/operational-tasks`**. **UI (Card 8):** **`GlobalAssistantProvider`** + **`GlobalAssistantShell`** in **`AdminV2Shell`**; **`TopNavBar`** **Assistant** button opens the panel; **`TaskAssistOpportunityWorkspace`** hosts full flows; opportunity drawer supplies **`TaskAssistOpportunityLauncher`** context only — **not** drawer-owned Task Assist UI. Gated by **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`** (`web/lib/agent/taskAssist/taskAssistV1UiGate.ts`). Client fetch: **`taskAssistV11OpportunityApi.ts`**. **Propose** remains **deterministic** (no live OpenAI on that route — see propose route). Distinct from bottom **`AICommandSurfaceShell`** (job layout) and **AI log**. Sprint: **`docs/sprints/05_2026/task_assist_v1_1.md`**.
...
| **Task Assist V1 + V1.1** | V1/V1.1 admin routes (see sprint §12); **`web/lib/agent/taskAssist/**`**, **`executeCommunicationsSend`**; global UI: **`web/contexts/GlobalAssistantContext.tsx`**, **`web/app/adminV2/components/globalAssistant/*`**, **`TaskAssistOpportunityWorkspace.tsx`**, **`TaskAssistOpportunityLauncher.tsx`**; **`taskAssistV1UiGate.ts`**; tests **`web/tests/agent/taskAssist/**`**; **`docs/sprints/05_2026/task_assist_v1.md`**, **`docs/sprints/05_2026/task_assist_v1_1.md`**. |
