...
- **Task Assist V1.1 (Agent 2 — durable layer + AdminV2 command bar):** Same admin routes as above plus **`POST`/`GET /api/admin/ai/task-assist/proposals`**, approve/reject, **`GET /api/admin/ai/task-assist/entity-search`** (read-only target lookup), **`/api/admin/communication-scheduled-sends`** (+ **`process-due`**), **`/api/admin/operational-tasks`**. **UI (Card 9 complete):** Bottom **`AICommandSurfaceShell`** is the **primary** Task Assist home — **ambient** context (drawer/page), **explicit search** (entity-search + confirm), **deterministic intent routing** (`parseTaskAssistCommandIntent` — draft / schedule / reminder prefill), **workflow phrase block**; **Confirm target** always required before workspace; **never auto-send**. **`GlobalAssistantProvider`** holds **`currentContext`** + **`commandSurfaceMode`**; **`TaskAssistOpportunityWorkspace`** in the tray; drawer **`TaskAssistOpportunityLauncher`** sets context only. Gated by **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. **Propose** remains **deterministic** (no LLM on that route). Limitations: heuristics only, persons search deferred, approximate timing hints, no workflow config. Sprint: **`docs/sprints/05_2026/task_assist_v1_1.md`**.
...
| **Task Assist V1 + V1.1 (Agent 2)** | V1/V1.1 admin routes (sprint §12); **`web/lib/agent/taskAssist/**`** (incl. **`taskAssistCommandIntent.ts`**, **`taskAssistEntitySearchService.ts`**); command bar: **`web/app/adminV2/components/aiCommandSurface/AICommandSurfaceShell.tsx`**, **`web/lib/adminV2/aiCommandSurface/adminV2CommandBarEvents.ts`**, **`GlobalAssistantContext.tsx`**, **`TaskAssistOpportunityWorkspace.tsx`**, **`TaskAssistOpportunityLauncher.tsx`**; **`taskAssistV1UiGate.ts`**; tests **`web/tests/agent/taskAssist/**`**; **`docs/sprints/05_2026/task_assist_v1.md`**, **`docs/sprints/05_2026/task_assist_v1_1.md`**. |
