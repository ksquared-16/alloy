...
| **Workflow Assist (Agent #3)** | Workflow configuration, draft generation, maintenance/oversight, noisy-workflow detection — **disabled-by-default drafts**, human approval before save/apply | **No** — workflow-like NL → **`workflow_assist_notice`** in thread only; action cards **later** |
