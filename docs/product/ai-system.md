...
- **Workflow Assist V1 (Agent 3 — Cards 4–5 + Explain v1 + NL create + product loop):** **`POST /api/admin/ai/workflow-assist/propose`** / **`POST …/apply`** (admin-gated). **`GET /api/admin/ai/workflow-assist/explain`** — Explain v1 (`explain_engine: 1`). NL **create** routes to propose with **`workflows.metadata.scope`** from workspace route context (§15). Tour reminder applies a **log action scaffold** (disabled workflow). Dept/work-unit panels partition **scoped vs org-wide** workflows; **post-apply** refetches partitions via `alloy-adminv2-workflow-automation-refresh`; **Ask Workflow Assist** seeds the command bar from automation blocks; read cards support narrow **rename / description / disable** proposals with **edit_review** on the proposal card (§16). Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`** §12–16.
