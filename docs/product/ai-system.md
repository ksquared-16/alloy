...
- **Workflow Assist V1 (Agent 3 — Cards 4–5 + Explain v1 + NL create):** **`POST /api/admin/ai/workflow-assist/propose`** / **`POST …/apply`** (admin-gated). **`GET /api/admin/ai/workflow-assist/explain`** — Explain v1 (`explain_engine: 1`). NL **create** routes to propose (templates in sprint doc §14). Dept/work-unit **`AutomationWorkflowsBlock`** — org-wide workflow list + KPIs (no dept/WU FK on `workflows` yet). Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`** §12–14.
