...
- **Task Assist V1.1 (Agent 2 — durable layer + AdminV2 command bar):** Same admin routes as above plus **`POST`/`GET /api/admin/ai/task-assist/proposals`**, approve/reject, **`GET /api/admin/ai/task-assist/entity-search`** (read-only target lookup), **`/api/admin/communication-scheduled-sends`** (+ **`process-due`**), **`/api/admin/operational-tasks`**. **UI (Interaction Layer V1):** Bottom **`AICommandSurfaceShell`** is the **primary universal assistant** — **single input**, **no visible modes**, **`routeCommandSurface`** (Task Assist vs job layout vs Workflow Assist notice), **thread + action cards**, **slot-based entity/goal extract** (`commandSurfaceSlotExtract`), **ambient** context (drawer/page), **entity-search + confirm target** before operational actions; **`TaskAssistOpportunityWorkspace`** expands inside action cards only; **never auto-send**, **no bulk**, **no workflow config** in Task Assist. **`GlobalAssistantProvider`** holds **`currentContext`** + **`focusCommandBar`**. Gated by **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. **Propose** remains **deterministic** (no LLM on that route). Sprints: **`docs/sprints/05_2026/task_assist_v1_1.md`**, **`docs/sprints/05_2026/agent_interaction_layer_v1.md`**.
...
| **Task Assist V1 + V1.1 (Agent 2)** | V1/V1.1 admin routes (sprint §12); **`web/lib/agent/taskAssist/**`**; Interaction Layer: **`web/lib/adminV2/aiCommandSurface/commandSurfaceRouter.ts`**, **`commandSurfaceSlotExtract.ts`**, **`commandSurfaceThreadState.ts`**, **`CommandSurfaceThread.tsx`**; command bar: **`AICommandSurfaceShell.tsx`**, **`adminV2CommandBarEvents.ts`**, **`GlobalAssistantContext.tsx`**, **`TaskAssistOpportunityWorkspace.tsx`**, **`TaskAssistOpportunityLauncher.tsx`**; **`taskAssistV1UiGate.ts`**; tests **`web/tests/agent/taskAssist/**`**, **`web/tests/adminV2/commandSurface*.test.ts`**; **`docs/sprints/05_2026/task_assist_v1.md`**, **`docs/sprints/05_2026/task_assist_v1_1.md`**, **`docs/sprints/05_2026/agent_interaction_layer_v1.md`**. |
