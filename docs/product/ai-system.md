...
- **Workflow Assist V1 (Agent 3 — Cards 4–5 + Explain v1 + NL create + product loop + AI draft enrichment):** **`POST /api/admin/ai/workflow-assist/propose`** / **`POST …/apply`** (admin-gated). **`GET /api/admin/ai/workflow-assist/explain`** — Explain v1 (`explain_engine: 1`). NL **create** routes to propose with **`workflows.metadata.scope`** from workspace route context (§15). **AI draft enrichment V1** (§17): stub advisory enrichment → deterministic normalization → compact **operator review card** with **visual stepper** (§18–20); tour reminder preview uses documented merge paths `{{opportunity.metadata.tour_date}}` / `{{opportunity.metadata.tour_time}}` plus **`reminder_intent_v1`** metadata — not `{{contact_name}}`. **Propose** runs **duplicate detection** (advisory warning; create anyway allowed). **Command-surface navigation** uses `CommandSurfaceCardLink` (**Opening…**, collapses thread on navigate). **`/adminV2/workflows?workflow=`** opens a read/detail panel (scope, actions, reminder intent). Apply still writes **disabled** workflows only. Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`** §12–20.
- **Orchestrator + Task Assist V1.1 (Interaction Layer + Agent #2):** **Orchestrator** = bottom **`AICommandSurfaceShell`** — single input, **`routeCommandSurface`**, thread + clarification/candidate turns, routes to specialists; **does not execute** sends/schedules/tasks/workflows itself. **Task Assist** = comms/reminder/schedule intents. **Workflow Assist** = **`workflow_assist`** route → summary / failed-runs / enrollment / **Explain v0** (`explain_v0` intent → **`GET …/workflow-assist/explain`** when ambient opportunity context exists); propose/apply admin-gated. Gated by **`NEXT_PUBLIC_TASK_ASSIST_V1_ENABLED`**. Sprints: **`agent_interaction_layer_v1.md`**, **`task_assist_v1_1.md`**, **`workflow_assist_v1.md`**.
