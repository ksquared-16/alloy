...
- **Workflow Assist V1 (Agent 3 — Cards 4–5 + Explain v1):** **`POST /api/admin/ai/workflow-assist/propose`** / **`POST …/apply`** (admin-gated). **`GET /api/admin/ai/workflow-assist/explain`** — Explain v1: returns **`WorkflowOperationalTraceV1`** + explanation (`explain_engine: 1`) — correlates events, workflows, **`workflow_conditions`** (runtime eval), runs, action_runs, status transitions (`requireAdminOrOps`). **`GET …/capabilities`** gates propose/apply CTAs. Spec: **`docs/sprints/05_2026/workflow_assist_v1.md`** §12.
