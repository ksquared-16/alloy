...
| 4 | Activity is a Runtime consumer | **✗** | Work→Activity is local mode state; cockpit mount+fetches. PARTIAL (seed + prewarm + no remount), not a runtime commit. |
| 5 | Communications / Processing / Work Items / OI inherit Runtime | **✗** | All four are `openWorkspaceModal` mount+fetch modals; none touch attention/provisioning/commit. |
