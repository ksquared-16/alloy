...
| LEAK Provisioning domain→platform leaks (FocusPanelSubjectSnapshot + truth-key assembly) | Runtime Arch | High | — | ✓ | **CODE DONE / AWAITING BROWSER CERT** — generic `SubjectIdentityTruth` bag replaces the opportunity-shaped type through the reveal chain (7 files); domain composer declares the truth keys, platform forwards opaquely. tsc 0; +0 unit (84=84). **Loads-as-one/warm-<2s/no-flash browser cert pending slot3 auth refresh.** |
