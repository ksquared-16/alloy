...
| B1-dedup Background duplicate-request fix (drawer-recipients ×2 / family-workspace ×2 / queue-view-totals ×2) | Critical Path | High | — | ✓ | **READY** (Lane 1B — measure owner/trigger per request) |
| PROVE family_alerts dynamic-card test | Scalability | High | RegC-2, RegC-3 | ✗ | blocked (Lane 2B — after placement+loadingPolicy) |
| LEAK Provisioning domain→platform leaks (FocusPanelSubjectSnapshot + truth-key assembly) | Runtime Arch | High | — | ✓ | READY but **reveal-path cert-sensitive → batch w/ browser loop** (Lane 2C) |
...
| SC-1 Generalize subject contract (PoC type compiles for opportunity + child) | Scalability | **High (was Medium)** | — | ✓ | **READY — now SPECIFIED by the second-surface design** (parameterize `subject.type`/`grain` off the answer; typecheck-certifiable, EEC-free) |
