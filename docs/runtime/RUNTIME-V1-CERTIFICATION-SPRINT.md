...
**NEXT.** PRIMARY-USABLE ≈ PRIMARY-SETTLED ≈ ~6.6 s (the 5 grid cells fill together at first-meaningful — probe showed no Tier-3 grid churn; DEFERRED-DETAIL/tabs ≈ 12.5 s but off the primary path). The dominant residual is the **enriched-VM compose + bundle/hydrate**, i.e. **Runtime V1 Realization** (server-compose the Surface VM), which Kelly deferred to a dedicated session — the reveal-gate levers available in THIS sprint are now exhausted (bottleneck #1 already solved, #2 landed, #3 rejected). Recommend closing CP-1's in-sprint perf work here and routing the remaining first-meaningful reduction to Realization. Perf/browser measurement is EEC. _Below: the prior mission-priority note._
