...
- **Queue row preview actions:** `queue_definition.ui.row_preview.actions` selects which **slots** appear on each row (e.g. `open`, `call`, `email`). **Which** slots show is configuration; the **handlers** for built-in slots are code-defined today (e.g. `crm_tel` / `crm_mailto` for tel/mailto). A future **single “Message”** action that opens the **communications composer** with channel choice should be added as a **first-class configured action** (registry + handler), not as another one-off hardcoded branch beside Call/Email.
...
- **Specification:** [AI agent foundation](./ai-agent-foundation.md) — agent role, capability map, boundaries, API interaction model, safety and audit (contracts only; no runtime). [AI agent system contract](./ai-agent-system-contract.md) — typed identity, intents, validation layers, governance audit vs business events.
