...
test("the filing path no longer substitutes a default census", () => {
  // The substitution was `shape.artifactRefs.length ? ... : [Q15_CENSUS_ARTIFACT]`.
  assert.doesNotMatch(SRC, /\?\s*shape\.artifactRefs\s*:\s*\(shape\.actionKey === ACTION_TYPES\.DATABASE_READ_CENSUS/);
  assert.match(SRC, /census_query_required/,
    "filing must refuse a census that names no query");
});

test("artifactPathFrom returns null rather than the authority census", () => {
  // The single line that turned "no query" into "the most privileged query".
  assert.doesNotMatch(SRC, /return first \? String\(first\) : Q15_CENSUS_ARTIFACT;/);
  assert.match(SRC, /return first \? String\(first\) : null;/);
});
