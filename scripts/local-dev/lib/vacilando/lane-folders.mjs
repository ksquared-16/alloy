...
export function listLaneFolders(root = undefined) {
...
export function createLaneFolder({ name, nowMs = Date.now(), root = undefined } = {}) {
  const named = validateFolderName(name);
  if (!named.ok) return named;
  const store = readDevelopmentLaneStore(root);
  const folders = foldersOf(store);
  if (Object.values(folders).some((f) => f.name.toLowerCase() === named.name.toLowerCase())) {
...
export function assignLaneToFolder(laneId, folderId, { nowMs = Date.now(), root = undefined } = {}) {
  ...
  if (folderId != null && folderId !== "") {
    if (!foldersOf(store)[String(folderId)]) return { ok: false, error: "folder_not_found" };
