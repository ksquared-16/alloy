select 'coverage_lifecycle_final' as question_id, 'row' as kind,
  json_build_object(
    'step', 'final_after_cancel_reason_repair',
    'lineage', (select coalesce(json_agg(json_build_object(
                    ...
                    'source', a.source_key) order by a.created_at, a.id),'[]'::json)
    ...
    'effective_by_employment', (select coalesce(json_agg(r.id order by r.id),'[]'::json)
                ...
                cross join lateral public.staff_coverage_effective_for_employment(
                    e.org_id, e.id, date '2027-04-02', date '2027-04-02') r
                where e.id = '14a42234-82cf-4b3e-87d4-b43175e9b06c'),
