-- Read-only census (certification stack): does `commercial_policy_exceptions` physically exist on the
...
--
-- ── WHY A SECOND ARTIFACT RATHER THAN A RE-FILE ──────────────────────────────
-- `database.read_census` dedupes on query hash within a run, so re-filing the
-- deployed-database artifact would replay that answer instead of asking this
-- question. This asks a DIFFERENT database the same facts, and is therefore a
-- different question with a different identity — not a retry.
--
-- The migration resolved to `certification_local` (the local cert stack) because
-- `certification` maps there; the deployed database is a separate class that
-- only takes migrations already on origin/staging. This measures the stack the
-- apply actually wrote to.
