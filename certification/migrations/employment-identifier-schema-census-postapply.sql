-- POST-APPLY reading. Read-only census: does the staging database physically carry Slice 1's schema?
--
...
--
-- This is the AFTER half of the pair. The BEFORE half (all false, pre-merge) was
-- filed from the identical questions in employment-identifier-schema-census.sql.
-- The bytes differ ONLY in this header, and they must: database.read_census
-- dedupes on the sha256 of the query within a run, so re-filing the same artifact
-- would REPLAY the pre-merge answer and render a successful migration as a
-- failure. Different bytes, genuine measurement.
--
-- After the staging apply it
