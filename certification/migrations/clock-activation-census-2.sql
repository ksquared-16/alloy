-- Second reading: does the pg_cron job ACTUALLY fire? A census dedupes within a
-- run, so this is a distinct artifact rather than the same one filed twice.
