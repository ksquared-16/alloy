-- POST-PROMOTION census. Phase 2 of the certification gate.
--
-- Taken after PR 652 merged, the toolkit was reinstalled from the merge commit, and the fixture was
-- reset once / built once / verified twice from the INSTALLED build rather than a lane-private one.
--
-- Its first section repeats the committed pre-fixture baseline's questions verbatim, because a delta
-- is only meaningful against the same questions. A separate file (and therefore a separate hash) is
-- deliberate: the census runner caches by query hash, so re-sending the baseline file would return
-- the baseline's own stored answer and report a delta of zero no matter what the database now holds.
--
-- The second section adds the dimensions Phase 2 names that the baseline never asked -- households,
-- people, children, agreements, placements, schedules. Those have no prior reading, so they are
-- reported as absolute counts and labelled as such rather than dressed up as deltas.
--
...
from (
    ...
    -- Dimensions the baseline never asked. Absolute counts, not deltas -- there is nothing to
    -- subtract from, and presenting them as deltas would invent a comparison.
    select 'absolute_population'::text,
           json_build_object(
               'households', (select count(*) from public.customers),
    ...
    -- The fixture namespace itself, so its footprint is separable from the tenant's.
    select 'fixture_namespace_footprint'::text,
           json_build_object(
               'people', count(distinct p.id),
               'households', count(distinct cp.customer_id),
               'children', count(distinct cm.id)
           )::text
    from public.persons p
    left join public.customer_persons cp on cp.person_id = p.id
    left join public.customer_members cm on cm.customer_id = cp.customer_id
    where p.email like '%@enrollment-cert.alloy.invalid'
) census
