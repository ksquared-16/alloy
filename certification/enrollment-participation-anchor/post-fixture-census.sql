-- POST-FIXTURE snapshot. Byte-for-byte the same questions as certification-baseline-census.sql,
-- copied rather than re-run in place so the baseline results file survives and the delta is a plain
-- file diff any reader can check. Every difference must be fixture-owned.
