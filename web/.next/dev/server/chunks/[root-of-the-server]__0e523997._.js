...
"[project]/app/api/admin/contact-options/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    const { data: rows, error } = await supabase.from("contacts").select("id, first_name, last_name, email, phone, company_name").eq("org_id", ctx.orgId).eq("customer_id", customerId).order("last_name", {
        ascending: true,
        nullsFirst: false
    }).order("first_name", {
        ascending: true,
    ...
    const contacts = (rows ?? []).map((r)=>{
        ...
        const label = [
            first,
            last
        ].filter(Boolean).join(" ") || company || email || phone || r.id;
        return {
            id: r.id,
            label: label.trim() || r.id
        };
    });
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        contacts
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0e523997._.js.map