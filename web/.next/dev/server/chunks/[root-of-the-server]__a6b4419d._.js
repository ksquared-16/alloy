...
function utcFallbackOperationalDayContext(refUtc) {
    const dayBounds = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$admin$2f$orgLocalDayBounds$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getOrgLocalTodayUtcBounds"])(__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$admin$2f$timezoneContract$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["UTC_FALLBACK_IANA"], refUtc);
    return {
        dayBounds,
        calendar_meta: {
            calendar_type: "operational_day",
            timezone_effective: __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$admin$2f$timezoneContract$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["UTC_FALLBACK_IANA"],
...
function planContextOrUtcFallback(ctx, refUtc) {
    return ctx ?? utcFallbackOperationalDayContext(refUtc);
}
async function resolveOperationalDayPlanContext(supabase, orgId, refUtc) {
    const { iana, source } = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$admin$2f$timezoneContract$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["fetchOperationalTimezoneForOrg"])(supabase, orgId);
    const dayBounds = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$admin$2f$orgLocalDayBounds$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getOrgLocalTodayUtcBounds"])(iana, refUtc);
    return {
        dayBounds,
        calendar_meta: {
            calendar_type: "operational_day",
...
function queueUsesOperationalCalendarDateFilter(queue) {
    return queue.filters.some((f)=>f.type === "date" && (f.operator === "today" || f.operator === "past_due"));
}
function buildJobPlan(queue, ctx) {
    ...
    return {
        ops,
        sort,
        calendar_meta: useCalendarMeta ? resolved.calendar_meta : undefined
    };
}
function buildOpportunityPlan(queue, now = new Date(), ctx) {
...
function jobFilterToOps(f, dayBounds) {
    switch(f.type){
        case "status":
            {
                // jobs.status_key IN (...)
...
async function enrichOpportunityRows(params) {
    ...
    const [personsRes, contactsRes, customersRes, ocmsRes, defs] = await Promise.all([
        personIds.length ? supabase.from("persons").select("id, first_name, last_name, email, phone, date_of_birth").eq("org_id", orgId).in("id", personIds) : Promise.resolve({
            data: [],
            error: null
        }),
    ...
    ].filter((id)=>!primaryPersonIdSet.has(id));
    const { data: childPersons } = !previewLite && childPersonIdsToFetch.length > 0 ? await supabase.from("persons").select("id, date_of_birth").eq("org_id", orgId).in("id", childPersonIdsToFetch) : {
        data: []
    };
    ...
    for (const row of ocmsRes.data ?? []){
        ...
        const pid = cm && typeof cm === "object" ? String(cm.person_id ?? "").trim() : "";
        const canonicalDob = pid ? childDobByPersonId.get(pid) ?? String(personById.get(pid)?.date_of_birth ?? "").trim() : "";
 ...
 */ function buildOpportunityNeedsAttentionCandidateOrExpr(now) {
    const stale3d = toIso(subtractDays(now, 3));
    const stale2d = toIso(subtractDays(now, 2));
    return [
        `updated_at.lt.${stale3d}`,
...
async function loadOpportunityNeedsAttentionRows(params) {
    ...
    let q = params.supabase.from("opportunities").select("id, name, title, status_key, customer_id, primary_person_id, primary_contact_id, work_unit_id, metadata, created_at, updated_at").eq("org_id", params.orgId).eq("work_unit_id", params.workUnitId).or(candidateOr);
...
function opportunityFilterToOps(f, now, dayBounds) {
    switch(f.type){
        case "status":
            {
                if (f.operator !== "in") {
...
async function getWorkUnitQueueSummaries(params) {
    ...
    const factories = def.queues.map((q, queueIndex)=>async ()=>{
            ...
            let rowsEnriched = 0;
            const finish = (summary, calendar_meta)=>{
            ...
            if (def.entity_type === "job") {
                ...
                const baseScope = supabase.from("jobs").eq("org_id", params.orgId).eq("work_unit_id", params.workUnitId);
                if (!includePreviews) {
                    const tC0 = Date.now();
                    const countQ = applyOpsToJobQuery(baseScope.select("id", {
                        count: countSel,
                        head: true
                    }), ops);
...
"[project]/app/api/admin/queues/[workUnitId]/[queueKey]/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function parseLimitOffset(searchParams) {
    ...
    if (offsetRaw) {
        ...
        if (!Number.isFinite(n)) throw new __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$queues$2f$QueueService$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["QueueServiceError"]("offset must be a number", 400, "VALIDATION_FAILED");
        const i = Math.floor(n);
        if (i < 0) throw new __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$queues$2f$QueueService$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["QueueServiceError"]("offset must be >= 0", 400, "VALIDATION_FAILED");
        offset = i;
    }
    return {
        limit,
...
function parseQueueItemsCountOptions(searchParams) {
    const countRaw = (searchParams.get("count_mode") ?? "").trim().toLowerCase();
    const omitTotalCount = searchParams.get("omit_total_count") === "true" || countRaw === "omit" || countRaw === "none";
    if (omitTotalCount) {
        return {
    ...
    throw new __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$queues$2f$QueueService$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["QueueServiceError"]("count_mode must be exact, planned, or omit", 400, "VALIDATION_FAILED");
}
async function GET(request, context) {
    ...
    if (!workUnitId) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        error: "Missing workUnitId"
    }, {
        status: 400
    });
    if (!queueKey) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        error: "Missing queueKey"
    }, {
        status: 400
    });
    const t0 = Date.now();
    try {
        ...
        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$queues$2f$QueueService$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getWorkUnitQueueItems"])({
            orgId: ctx.orgId,
            workUnitId,
            queueKey,
            limit,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a6b4419d._.js.map