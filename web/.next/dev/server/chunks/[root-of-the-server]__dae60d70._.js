...
"[project]/lib/communications/composerChannels.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";

/** Card 12 — derive which outbound channels appear in composer from binding rows (no secrets). */ __turbopack_context__.s([
    "activeOutboundBindings",
    ()=>activeOutboundBindings,
    "availableComposerChannels",
    ()=>availableComposerChannels
]);
function isSmsBindingReady(b) {
...
function isEmailBindingReady(b) {
...
function availableComposerChannels(bindings) {
    const out = new Set();
    out.add("in_app");
...
function activeOutboundBindings(bindings, channel) {
    const ch = channel.toLowerCase();
    if (ch === "in_app") return [];
    return bindings.filter((b)=>(b.status || "").toLowerCase() === "active" && (b.channel || "").toLowerCase() === ch && (ch === "sms" ? isSmsBindingReady(b) : ch === "email" ? isEmailBindingReady(b) : false));
}
}),
...
"[project]/app/api/admin/communications/bindings/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$composerChannels$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/communications/composerChannels.ts [app-route] (ecmascript)");
...
function sanitizeBindings(raw) {
    return raw.map((b)=>{
        ...
        const fromEmail = cfg && typeof cfg.from_email === "string" ? String(cfg.from_email).slice(0, 120) : null;
        return {
            id: b.id,
            channel: b.channel,
            scope: b.scope ?? null,
            ...
            /** Whether composer treats binding as actionable (SMS secret + Resend-ready). Derived — not raw secret. */ ready_for_composer: Boolean(typeof b.secret_ref === "string" && b.secret_ref.trim().toLowerCase() !== "" && b.secret_ref.trim().toLowerCase() !== "unconfigured"),
...
async function GET() {
    ...
    const { data, error } = await supabase.from("communication_provider_bindings").select("id, channel, scope, location_id, display_label, provider, status, is_primary, secret_ref, inbound_to_e164, config").eq("org_id", ctx.orgId).order("updated_at", {
        ascending: false
    });
    ...
    const list = data ?? [];
    const channels_available = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$composerChannels$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["availableComposerChannels"])(list);
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        bindings: sanitizeBindings(list),
        channels_available,
        selectable_by_channel: {
            sms: sanitizeBindings((0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$composerChannels$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["activeOutboundBindings"])(list, "sms")),
            email: sanitizeBindings((0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$composerChannels$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["activeOutboundBindings"])(list, "email"))
        },
        permission_stub: {
            gate: "admin_or_ops",
...
//# sourceMappingURL=%5Broot-of-the-server%5D__dae60d70._.js.map