...
"[project]/app/api/admin/communications/threads/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
;
;
/** Match /api/admin/activity entity_type normalization. */ function normalizeEntityTypeParam(raw) {
...
async function GET(request) {
    ...
    if (!entityType || !entityId || !UUID_RE.test(entityId)) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: "entity_type and valid entity_id (uuid) are required"
        }, {
            status: 400
        });
    }
    ...
    const { data: threads, error } = await supabase.from("communication_threads").select("id, org_id, channel, recipient_key, created_at, updated_at, metadata").eq("org_id", ctx.orgId).eq("primary_entity_type", entityType).eq("primary_entity_id", entityId).order("updated_at", {
        ascending: false
    }).limit(limit);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d98025b7._.js.map