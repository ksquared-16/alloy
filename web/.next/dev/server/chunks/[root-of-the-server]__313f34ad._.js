...
"[project]/app/api/admin/record-layouts/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    if (!entityType || !ALLOWED.has(entityType)) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: "entity_type must be job, schedule, or opportunity"
        }, {
            status: 400
        });
    }
    ...
    const { data: orgRow, error: orgErr } = await supabase.from("record_drawer_layouts").select("id, org_id, entity_type, surface, key, config_json, is_active, created_at, updated_at").eq("org_id", ctx.orgId).eq("entity_type", entityType).eq("surface", "drawer").eq("key", "default").eq("is_active", true).maybeSingle();
    if (orgErr) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
    ...
    if (orgRow) {
        ...
        const layout = {
            id: orgRow.id,
            entity_type: orgRow.entity_type,
            key: orgRow.key,
            config_json: orgRow.config_json ?? {},
            is_active: Boolean(orgRow.is_active),
            created_at: orgRow.created_at ?? new Date().toISOString()
        };
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            layouts: [
                layout
    ...
    const { data, error } = await supabase.from("record_layouts").select("id, entity_type, key, config_json, is_active, created_at").eq("entity_type", entityType).eq("is_active", true).order("created_at", {
        ascending: true
    });
    if (error) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__313f34ad._.js.map