...
"[project]/lib/communications/drawerEmailRecipients.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function fetchOpportunityDrawerEmailRecipients(supabase, orgId, opportunityId) {
    const { data: opp } = await supabase.from("opportunities").select("primary_person_id").eq("id", opportunityId).eq("org_id", orgId).maybeSingle();
    const primaryPid = opp && typeof opp.primary_person_id === "string" ? opp.primary_person_id.trim() : null;
    const { data: opRows } = await supabase.from("opportunity_persons").select("person_id, role_type").eq("org_id", orgId).eq("opportunity_id", opportunityId);
    ...
    const { data: people } = await supabase.from("persons").select("id, first_name, last_name, full_name, email").eq("org_id", orgId).in("id", [
        ...pidSet
    ]);
    const rows = [];
    for (const p of people ?? []){
        ...
        rows.push({
            ...
            relationship_hint: pid === primaryPid ? "Primary person" : roleMap.has(pid) ? roleMap.get(pid) ?? "Linked" : "Linked person",
    ...
    /** Default: primary with email else first deterministic by name then id. */ const sortedForDefault = [
        ...rows
    ].sort((a, b)=>{
    ...
    let defaultPid = primaryPid && byId.has(primaryPid) ? primaryPid : sortedForDefault[0]?.person_id ?? null;
    rows.forEach((r)=>{
...
async function fetchJobDrawerEmailRecipients(supabase, orgId, jobId) {
    const { data: job } = await supabase.from("jobs").select("primary_person_id, customer_id, opportunity_id").eq("id", jobId).eq("org_id", orgId).maybeSingle();
    if (!job) return [];
    const primaryPidRaw = job.primary_person_id;
    const customerId = job.customer_id;
    const oppId = job.opportunity_id;
    const primaryPid = typeof primaryPidRaw === "string" && primaryPidRaw.trim() ? primaryPidRaw.trim() : null;
    const pidSet = new Set();
    const hintMap = new Map();
    if (customerId) {
        const { data: cpRows } = await supabase.from("customer_persons").select("person_id, is_primary, role_type").eq("org_id", orgId).eq("customer_id", customerId);
        const rowsCp = (cpRows ?? []).sort((a, b)=>{
            const sc = (r)=>Number(!!r.is_primary && r.role_type === "primary_contact") * 3 + Number(!!r.is_primary) * 2 + Number(r.role_type === "primary_contact");
            const d = sc(b) - sc(a);
            if (d !== 0) return d;
            return 0;
        });
        for (const r of rowsCp){
            ...
            if (!pid) continue;
            pidSet.add(pid);
            if (!hintMap.has(pid)) hintMap.set(pid, r.role_type === "primary_contact" ? "Primary contact role" : r.is_primary ? "Primary on customer" : "Household member");
        }
    }
    if (oppId) {
        const { data: oppRows } = await supabase.from("opportunity_persons").select("person_id, role_type").eq("org_id", orgId).eq("opportunity_id", oppId);
    ...
    let defaultPid = primaryPid && rows.some((r)=>r.person_id === primaryPid) ? primaryPid : sortedForDefault[0]?.person_id ?? null;
    rows.forEach((r)=>{
        r.is_suggested_default = r.person_id === defaultPid;
    });
    rows.sort((a, b)=>{
...
async function assertRecipientPersonEligibleForDrawerEmail(supabase, orgId, entityType, entityId, personId) {
    const list = entityType === "jobs" ? await fetchJobDrawerEmailRecipients(supabase, orgId, entityId) : await fetchOpportunityDrawerEmailRecipients(supabase, orgId, entityId);
    return list.some((r)=>r.person_id === personId);
}
async function getPersonEmailOrNull(supabase, orgId, personId) {
    const { data } = await supabase.from("persons").select("email").eq("id", personId).eq("org_id", orgId).maybeSingle();
    return trimEmail(data?.email);
}
}),
"[project]/app/api/admin/communications/drawer-recipients/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabaseAdmin$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/supabaseAdmin.ts [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$drawerEmailRecipients$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/communications/drawerEmailRecipients.ts [app-route] (ecmascript)");
;
;
;
;
...
async function GET(request) {
    ...
    if (!entityType) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: "entity_type must be opportunities or jobs"
        }, {
            status: 400
        });
    }
    ...
    if (!(await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$admin$2f$assertRowOrg$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["assertRowOrg"])(supabase, table, entityId, ctx.orgId)).ok) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
    ...
    const recipients = entityType === "jobs" ? await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$drawerEmailRecipients$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["fetchJobDrawerEmailRecipients"])(supabase, ctx.orgId, entityId) : await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$communications$2f$drawerEmailRecipients$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["fetchOpportunityDrawerEmailRecipients"])(supabase, ctx.orgId, entityId);
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        recipients,
        source_note: entityType === "jobs" ? "persons from customer_persons (job customer_id), optional opportunity_persons via job.opportunity_id, union primary_person_id; contact never used as recipient anchor" : "persons from opportunity_persons union primary_person_id; contact fields never used as recipient anchor"
    });
}
}),
];

//# sourceMappingURL=%5Broot-of-the-server%5D__b26d7c4b._.js.map