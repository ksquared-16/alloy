...
"[project]/app/api/admin/agent/v1/activity/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
;
;
function agentV1RecordLayoutEnabled() {
...
async function GET(request) {
    ...
    if (!agentV1RecordLayoutEnabled()) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ...
            message: "Agent v1 record layout is disabled"
        }, {
            status: 403
        });
    }
    ...
    const { data: audits, error: auditErr } = await supabase.from("agent_v1_record_layout_apply_audit").select("result_id, proposal_id, org_id, user_id, record_overview_layout_id, terminal_status, applied_config_version, created_at").eq("org_id", ctx.orgId).order("created_at", {
        ascending: false
    }).limit(limit);
    if (auditErr) {
    ...
    const proposalIds = [
        ...new Set(list.map((a)=>a.proposal_id))
    ];
    const { data: proposals, error: propErr } = await supabase.from("agent_v1_record_layout_proposals").select("proposal_id, request_id, correlation_id, intent_json, record_overview_layout_id, created_at").in("proposal_id", proposalIds).eq("org_id", ctx.orgId);
    if (propErr) {
    ...
    const propMap = new Map((Array.isArray(proposals) ? proposals : []).map((p)=>[
            p.proposal_id,
            p
        ]));
    const layoutIds = [
        ...new Set(list.map((a)=>a.record_overview_layout_id))
    ];
    const { data: layouts, error: layErr } = await supabase.from("record_overview_layouts").select("id, entity_type, surface").in("id", layoutIds).eq("org_id", ctx.orgId);
    if (layErr) {
        console.error("[ai-activity] layouts:", layErr);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ok: false,
            error: "LIST_FAILED",
            message: layErr.message
        }, {
            status: 500
        });
    }
    const layoutMap = new Map((Array.isArray(layouts) ? layouts : []).map((l)=>[
            l.id,
            l
        ]));
    const items = list.map((a)=>{
        ...
        const outcomeSummary = a.terminal_status === "success" ? `Applied config v${a.applied_config_version}` : `Status: ${a.terminal_status}`;
        return {
            id: a.result_id,
...
function mapTerminalStatus(terminal) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__04b7025a._.js.map