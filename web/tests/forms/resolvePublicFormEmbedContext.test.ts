...
vi.mock("@/lib/forms/packets/formPacketService", async (importOriginal) => {
    ...
    return {
        ...
        loadPacketDefinitionStepSummaries: (...args: unknown[]) => hoisted.mockStepSummaries(...args),
...
describe("resolvePublicFormEmbedContext", () => {
    beforeEach(() => {
        ...
        hoisted.mockStepSummaries.mockResolvedValue({
            data: [{ sequence_index: 0, form_name: "Step One" }],
            error: null,
        });
    });
    ...
    it("packet in progress resolves active step envelope and packet meta", async () => {
        ...
        expect(r.value.packet?.step_summaries).toEqual([{ sequence_index: 0, form_name: "Step One" }]);
        expect(hoisted.mockStepSummaries).toHaveBeenCalled();
        expect(hoisted.mockResolveEnvelope.mock.calls[0][4]).toEqual({ followLatestPublished: true });
    });
    ...
    it("packet_step_version_policy pinned passes followLatestPublished false to envelope resolve", async () => {
        hoisted.mockResolveLink.mockResolvedValueOnce({
            ok: true as const,
            value: {
                ...baseLink,
                linkMetadata: {
                    ...
                    packet_definition_id: PID,
                    packet_step_version_policy: "pinned",
                },
            },
        });
        hoisted.mockEnsure.mockResolvedValueOnce({
            ...
            data: [
                {
                    ...
                    sequence_index: 0,
                    form_definition_id: "ffffffff-ffff-4fff-8fff-ffffffffffff",
                    pinned_form_definition_version_id: "legacy-pin",
                },
            ],
            error: null,
        });
        hoisted.mockResolveEnvelope.mockResolvedValueOnce({
            envelope: {
                formDefinitionId: "ffffffff-ffff-4fff-8fff-ffffffffffff",
                formDefinitionVersionId: "pinned-ver",
                schemaJson: { schema_version: 1, title: "Pinned", sections: [], fields: [] },
                pdfMappingJson: null,
                formKey: "step",
                formName: "Step One",
                formKind: "family",
        ...
        const r = await resolvePublicFormEmbedContext({} as SupabaseClient, "tok");
        expect(r.ok).toBe(true);
        expect(hoisted.mockResolveEnvelope.mock.calls[0][4]).toEqual({ followLatestPublished: false });
    });
});
