...
describe("VM drawer transition wiring", () => {
    it("AdminEntityDrawer routes by display drawer during VM transitions", async () => {
        const { readFileSync } = await import("node:fs");
        const { join, dirname } = await import("node:path");
        const { fileURLToPath } = await import("node:url");
        const webRoot = join(dirname(fileURLToPath(import.meta.url)), "../../../");
        const router = readFileSync(join(webRoot, "components/admin/AdminEntityDrawer.tsx"), "utf8");
        expect(router).toContain("resolveVmDrawerDisplayRoute");
        expect(router).not.toContain("resolveVmDrawerRuntimeRoute(drawer");
    });

    it("VM runtimes gate cold shell and headers on vmMatchesRender", async () => {
        const { readFileSync } = await import("node:fs");
        const { join, dirname } = await import("node:path");
        const { fileURLToPath } = await import("node:url");
        const webRoot = join(dirname(fileURLToPath(import.meta.url)), "../../../");
    ...
    it("PersonsDrawerVmRuntime keeps committed title until VM matches render drawer", async () => {
        const { readFileSync } = await import("node:fs");
        const { join, dirname } = await import("node:path");
        const { fileURLToPath } = await import("node:url");
        const webRoot = join(dirname(fileURLToPath(import.meta.url)), "../../../");
