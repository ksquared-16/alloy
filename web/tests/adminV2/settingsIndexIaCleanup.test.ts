...
describe("Settings index IA — Configuration Runtime domains", () => {
    ...
    it("Processes tile lives under Operations after Automation", () => {
        ...
        expect(labels.slice(0, 2)).toEqual(["Automation", "Processes"]);
        ...
        expect(operations?.items.find((i) => i.href === "/organization/commands")).toBeUndefined();
        expect(operations?.items.find((i) => i.href === "/admin/workflows")?.emphasis).toBe(true);
    });
