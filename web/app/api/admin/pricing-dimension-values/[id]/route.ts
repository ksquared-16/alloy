...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const supabase = createAdminClient();
    const { data, error } = await supabase.from("pricing_dimension_values").update(updates).eq("id", id).eq("org_id", ctx.orgId).select("id, value_label, value_key, sort_order, is_active, updated_at").single();
