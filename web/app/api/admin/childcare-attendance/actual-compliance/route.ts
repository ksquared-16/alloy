...
import { fetchActualComplianceReadModel } from "@/lib/childcareOperational/attendance/fetchActualComplianceReadModel";
import {
...
/**
 * Read-only actual occupancy / staffing / compliance over L4 attendance facts.
 * Observational; staff data is optional (gaps are placeholders until staff
 * scheduling exists). Authors nothing.
 */
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const result = await fetchActualComplianceReadModel(supabase, {
            orgId: ctx.orgId,
            siteLocationId,
            dateStart,
            dateEnd,
