...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const planTemplateIds = (freqsRes.data ?? []).map((f: Record<string, unknown>) => f.service_plan_template_id).filter(Boolean) as string[];
        const plansRes = planTemplateIds.length ? await supabase.from("service_plan_templates").select("id, plan_name").in("id", planTemplateIds) : { data: [] };
        const planMap = new Map((plansRes.data ?? []).map((p: Record<string, unknown>) => [p.id as string, (p.plan_name as string) ?? null]));
        const freqToPlan = new Map((freqsRes.data ?? []).map((f: Record<string, unknown>) => [f.id as string, f.service_plan_template_id as string]));
        const freqLabelMap = new Map((freqsRes.data ?? []).map((f: Record<string, unknown>) => [f.id as string, (f.frequency_label as string) ?? null]));
        const planNameByFreqId = new Map<string, string>();
        ...
        const out: RecurringPriceRow[] = list.map((r: Record<string, unknown>) => {
            ...
            return {
                ...
                _plan_template_name: freqId ? (planNameByFreqId.get(freqId) ?? null) : null,
        ...
        console.error("[pricing/recurring-prices]", e);
