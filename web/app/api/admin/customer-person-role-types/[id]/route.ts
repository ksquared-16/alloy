...
/**
 * Update / delete a single customer-person role type.
 *
...
/** PATCH: update customer_person_role_type. Admin only. System types: key not editable (label, description, sort_order, is_active only). */
export async function PATCH(
    ...
    if (fetchErr || !existing) {
        return apiError("NOT_FOUND", "Role type not found", 404, undefined, { request });
    }
    ...
    logAdminAudit({
        entity: "customer_person_role_types",
        id,
