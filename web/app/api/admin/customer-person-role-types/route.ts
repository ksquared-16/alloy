...
/**
 * Configurable customer-person role types (list/create).
 *
...
export async function GET(request: NextRequest) {
    ...
    if (showAll) {
        ...
        // effective view but no org industry: show all active rows
...
export async function POST(request: NextRequest) {
    ...
    if (error) {
        ...
        if (code === "23505") {
            return apiError(
                "CONFLICT",
                "A role type with this key already exists for this org",
                409,
                undefined,
                { request }
            );
