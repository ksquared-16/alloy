...
export async function DELETE(
    ...
    const { error } = await supabase.from("pricing_dimensions").delete().eq("id", id).eq("org_id", ctx.orgId);
    if (error) {
        const msg = error.code === "23503" ? "Cannot delete: dimension is in use." : error.message;
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const supabase = createAdminClient();
    const { data, error } = await supabase.from("pricing_dimensions").update(updates).eq("id", id).eq("org_id", ctx.orgId).select("id, dimension_name, dimension_key, vertical_id, is_active, updated_at").maybeSingle();
