...
const PERSON_SYSTEM_KEYS = ["first_name", "last_name", "email", "phone"] as const;
...
export async function PATCH(
    ...
    const customKeys = Object.keys(body).filter(
        (k) => !PERSON_SYSTEM_KEYS.includes(k as (typeof PERSON_SYSTEM_KEYS)[number]) && k !== "full_name" && !k.startsWith("_")
    );
