...
const DEFAULT_BUCKET = "org_documents";
...
export async function POST(request: NextRequest) {
    ...
    if (!okEntity) {
        return NextResponse.json({ error: "Entity not found for this organization" }, { status: 404 });
    }

    const bucket = process.env.ADMIN_DOCUMENTS_BUCKET?.trim() || DEFAULT_BUCKET;
    ...
    if (upErr) {
        console.error("[DOCUMENTS_UPLOAD]", upErr);
        return NextResponse.json(
            { error: upErr.message || "Storage upload failed. Ensure the bucket exists and service role can write." },
            { status: 500 }
        );
    }
