...
export async function DELETE(
    ...
    const eligibility = await evaluateDeletionEligibility("addons", id, { orgId: ctx.orgId });
...
export async function PATCH(
    ...
    if (body.addon_name !== undefined) updates.addon_name = body.addon_name === "" ? null : body.addon_name;
    if (body.addon_key !== undefined) updates.addon_key = body.addon_key === "" ? null : body.addon_key;
    ...
    if (body.sort_order !== undefined) {
        ...
        updates.sort_order = Number.isNaN(n) ? 0 : Math.max(0, Math.round(n));
    }
    ...
    const { data, error } = await supabase
        ...
        .update(updates)
        .eq("id", id)
        .select("id, addon_name, addon_key, amount_cents, sort_order, is_active, updated_at")
        .single();
