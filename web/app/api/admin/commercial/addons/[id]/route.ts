...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (body.package_unit_count !== undefined) patch.package_unit_count = body.package_unit_count != null ? Number(body.package_unit_count) : null;
    if (body.package_unit_type !== undefined) patch.package_unit_type = body.package_unit_type != null ? String(body.package_unit_type).trim() || null : null;
    if (body.package_expires_days !== undefined) patch.package_expires_days = body.package_expires_days != null ? Number(body.package_expires_days) : null;
    ...
    const { data, error } = await supabase
        .from("commercial_addons").update(patch).eq("id", id).eq("org_id", ctx.orgId).select(SELECT_COLS).maybeSingle();
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const { id } = await params;
    const supabase = createAdminClient();
    const { data: existing } = await supabase.from("commercial_addons").select("id").eq("id", id).eq("org_id", ctx.orgId).maybeSingle();
    if (!existing) return NextResponse.json({ error: "Add-on not found" }, { status: 404 });

    const { error } = await supabase.from("commercial_addons").delete().eq("id", id).eq("org_id", ctx.orgId);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
