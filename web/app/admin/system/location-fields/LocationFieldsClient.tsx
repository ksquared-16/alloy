...
export default function LocationFieldsClient({
    ...
    const [createSectionKey, setCreateSectionKey] = useState("property");
    ...
    const fetchItems = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await fetch("/api/admin/field-definitions?entity_type=location");
                                ...
                                {sortedItems.length === 0 ? (
                                    <tr>
                                        <td colSpan={canMutate ? 11 : 10} className="py-4 text-[#59678b]">
                                            No field definitions. Add fields (e.g. square_footage, bedrooms) for quote-start and admin.
                                        </td>
                                    </tr>
                                ) : (
                                    sortedItems.map((row) => (
            ...
            {createOpen && (
                ...
                >
                    <div
                        ...
                        onClick={(e) => e.stopPropagation()}
                    >
                        <h3 className="mb-4 text-lg font-semibold text-[#31394d]">Add custom location field</h3>
                        <div className="space-y-3">
                            <div>
                                <label className="block text-xs font-medium text-[#59678b] mb-0.5">Label</label>
                                <input
