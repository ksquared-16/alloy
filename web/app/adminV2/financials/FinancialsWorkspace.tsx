...
/**
 * FINANCIALS — the operator workspace body.
 *
 * Overview answers one question — what financial work needs attention right now — and the Charges
 * section is where that work is picked up. Selecting a row opens the EXISTING Thread 2 account
 * detail; this file renders no financial figure of its own beyond the amount already written on the
 * charge, and computes none at all.
 *
 ...
 *
 * The queue is site-scoped when a site is selected. The account detail is NOT: Thread 2 answers for
 * a household across every site, and silently presenting an account-wide balance under a site
...
/** The Overview's activity tiles — the same counts the section band and the queue show. */
function overviewKpis(queue: FinancialWorkQueueState["data"]) {
    ...
    return [
        {
            ...
            icon: "inbox" as never,
            accent: null,
            formattedValue: String(counts?.actionable ?? 0),
            status: "ok",
            sourceKey: "financials.work_queue.actionable",
            drillHref: null,
        },
        {
            slot: 2,
            ...
            icon: "users" as never,
            accent: null,
            formattedValue: String(counts?.households ?? 0),
            status: "ok",
            sourceKey: "financials.work_queue.households",
            drillHref: null,
        },
    ];
}
    ...
    if (section === "overview") {
        return (
            <WorkspaceOverviewStack data-testid="financials-overview">
                <WorkspaceOverviewActivityBand testId="financials-overview-activity-kpis" busy={queue.loading}>
                    {overviewKpis(queue.data).map((kpi) => (
                        <SurfaceHeaderKpiCard key={kpi.slot} kpi={kpi as never} variant="work-unit" density="compact" />
                    ))}
                </WorkspaceOverviewActivityBand>
                <WorkspaceCard>
                    <p className="text-sm text-alloy-midnight/70" data-financials-overview-scope="true">
                        {/* The scope is stated because the counts above obey it. */}
                        {scopeLabel} · {queue.data?.counts.actionable ?? 0} draft{" "}
                        {(queue.data?.counts.actionable ?? 0) === 1 ? "charge is" : "charges are"} waiting to be posted.
                    </p>
                </WorkspaceCard>
            </WorkspaceOverviewStack>
        );
                ...
                {!selected ? (
                    ...
                    <div className="flex min-h-0 flex-1 flex-col" data-financials-detail={selected.chargeId}>
                        <div className="flex items-center justify-between gap-3 border-b border-alloy-stone/10 px-3 py-2">
                            ...
                            >
                                {posting ? "Posting…" : `Post ${money(selected.amountCents, selected.currencyCode)}`}
                            </button>
                        </div>
                        {actionError ? (
