...
export default function AssignmentSubjectPicker({
    ...
    open: boolean;
    onClose: () => void;
    onChoose: (subject: AssignmentSubjectChoice) => void;
}) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [childRes, staffRes] = await Promise.all([
                fetch("/api/admin/records/children?limit=200", { credentials: "include" }),
             ...
             */
            setChildren(
                ((childJson?.children ?? []) as { customerMemberId: string; displayName: string }[])
                    .map((r) => ({ customerMemberId: r.customerMemberId, name: r.displayName }))
                    .filter((r) => Boolean(r.customerMemberId)),
            );
            setStaff(
                ((staffJson?.staff ?? []) as { personId: string; displayName: string }[])
        ...
        >
            ...
            >
                <div className="flex items-center justify-between border-b border-alloy-stone/15 px-4 py-3">
                    <div>
                        <h2 className="text-[14px] font-semibold text-alloy-midnight">Create assignment</h2>
                        <p className="mt-0.5 text-[11.5px] text-alloy-slate">Who is this commitment for?</p>
                    </div>
                    <button type="button" onClick={onClose} aria-label="Close" className="text-alloy-slate">
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
                    {error ? (
                        ...
                        <p className="px-2 py-3 text-[12px] text-alloy-slate" data-assignment-subject-none="true">
                            {filter.trim() ? "No matches" : `No ${tab === "child" ? "children" : "staff"} yet`}
                        </p>
                    ) : (
                        rows.map((r: any) => {
