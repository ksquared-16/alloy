...
function DrawerRecordTabStripGateSkeleton({ tabCount = 4 }: { tabCount?: number }) {
    return (
        <div
            className="flex min-h-[2.875rem] flex-wrap gap-0.5 rounded-lg border border-admin-border bg-white p-0.5"
            aria-busy="true"
            aria-label="Loading record tabs"
        >
            {Array.from({ length: tabCount }, (_, i) => (
                <DrawerQuietSkeletonBar key={i} className="h-9 min-w-[4.25rem] flex-1 max-w-[6.5rem] rounded-md" />
            ))}
        </div>
    );
}
...
export default function AdminEntityDrawer() {
    ...
    useEffect(() => {
        ...
        if (drawer.type === "opportunities" && drawer.id !== "new") {
            ...
            const seedTrust = operTrustHintsFromQueueSeed(drawer.opportunityQueuePreviewSeed);
            setOpportunityOperTrustPreview(
                seedTrust.headline ?
                    {
                        headline: seedTrust.headline.slice(0, 140),
                        risk_urgency_hint:
                            seedTrust.urgency === "high" || seedTrust.urgency === "low" || seedTrust.urgency === "medium"
                                ? seedTrust.urgency
                                :   "medium",
                    }
                :   null
        ...
        const applyOpportunityBootstrap = (boot: OpportunityDrawerOperationalBootstrapResponse) => {
        ...
        const runLegacyEntityFetch = () => {
            ...
            if (process.env.NODE_ENV === "development" && drawer.type === "opportunities") {
                console.info("[drawer-bootstrap-fallback]", { id: drawer.id });
            }
        ...
        if (
            ...
            void fetchOpportunityDrawerOperationalBootstrap(drawer.id, drawer.opportunityWorkspaceContext, workspaceDataFetchInit())
                ...
                .catch(() => {
                    ...
                    setOpportunityBootstrapAppliedId(null);
                    setOpportunityBootstrapLayoutRow(null);
                    setOpportunityDrawerBootstrapLegacy(true);
    ...
    const runOpportunityFullHydrate = useCallback(() => {
        if (drawer.type !== "opportunities" || !drawer.id || drawer.id === "new") return;
        if (!opportunityRecordHydrationPending && opportunityDrawerBootstrapAppliedRef.current !== drawer.id) return;
    ...
    const recordChromeOpportunity = useRecordChromeConfig(
        ...
        opportunityBootstrapChromeSuppressed ?
            { bootstrapSeeded: true, seededLayout: opportunityBootstrapLayoutRow }
        :   undefined
    );
    const opportunityDrawerBootstrapPending =
        opportunityDrawerAdminV2Bootstrap &&
        !error &&
        !opportunityDrawerBootstrapLegacy &&
        (loading || opportunityBootstrapAppliedId !== drawer.id);
    const opportunityInquiryWorkflowDrawer =
        (opportunityDrawerAdminV2Bootstrap && !opportunityDrawerBootstrapLegacy) ||
        (drawer.type === "opportunities" &&
            recordChromeOpportunity.configResolved &&
            (recordChromeOpportunity.layout?.config_json as RecordLayoutConfigJson | null)?.inquiry_drawer_mode ===
                "workflow_v1");
    ...
    const opportunityDrawerQueueBootstrap =
        ...
        (drawer.type === "opportunities" &&
            !!drawer.id &&
            drawer.id !== "new" &&
            Boolean(drawer.opportunityQueuePreviewSeed));
    ...
    useEffect(() => {
        ...
        const sourceSurface =
            opportunityBootstrapAppliedId === drawer.id ? ("opportunity_drawer" as const)
            : drawer.opportunityQueuePreviewSeed ? ("queue" as const)
        ...
        globalAssistant.setAssistantContext(
            buildOpportunityOperationalContext({
                ...
                queuePreviewSeed:
                    opportunityBootstrapAppliedId === drawer.id ? null : (drawer.opportunityQueuePreviewSeed ?? null),
                opportunitySingular,
                sourceSurface,
            })
        );
    ...
    const opportunityDrawerShellSettled =
        ...
        drawer.id !== "new" &&
        drawerReady &&
        (opportunityBootstrapChromeSuppressed || recordChromeOpportunity.configResolved) &&
        !!overviewData &&
        !(overviewData as { _create?: boolean })._create;
    ...
    useEffect(() => {
        if (drawer.type !== "opportunities" || !drawer.id || drawer.id === "new") {
            setOpportunityTourBookingsId(null);
            return;
        }
        if (!opportunityDrawerSecondaryReady) {
            setOpportunityTourBookingsId(null);
            return;
        }
    ...
    const opportunityDrawerPreviewSubtitle = useMemo(() => {
        ...
        const trustLine = opportunityOperTrustPreview?.headline?.trim() ?? "";
        const previewSub = seed?.subtitle?.trim();
        const recordHint = seed?.recordNumberHint?.trim();
        const metaParts = [previewSub, recordHint].filter(Boolean);
        const meta = metaParts.length ? metaParts.join(" · ") : "";
        if (trustLine && meta) return `${trustLine} · ${meta}`;
        ...
        !opportunityDrawerOverviewRevealReady
            ? opportunityQueuePreviewSeed?.title?.trim() ||
              (opportunityDrawerQueueBootstrap
                  ...
                  : "Inquiry")
            : drawerGateLoading && opportunityQueuePreviewSeed?.title?.trim()
              ? opportunityQueuePreviewSeed.title.trim()
              : drawerGateLoading
    ...
    const headerSubtitleResolved =
        drawer.type === "opportunities" && opportunityInquiryWorkflowDrawer ? (
            ...
            <div className="mt-0.5 space-y-1.5">
                ...
                {overviewData &&
                !(overviewData as { _create?: boolean })._create &&
                (overviewData as Record<string, unknown>)._operational_attention != null ? (
                    ...
                    />
                ) : opportunityOperTrustPreview?.headline?.trim() && !opportunityFullHydrateApplied ? (
                    <p className="text-[11px] leading-snug text-alloy-midnight/70" data-drawer-oper-trust-preview="true">
                    ...
                    <DrawerRecordGateSkeleton
                        ...
                        modalOpportunityClassic={
                            !!(
                                opportunityRecordChromeBodyShell &&
                                ((recordChromeOpportunity.configResolved && !opportunityInquiryWorkflowDrawer) ||
                                    (opportunityDrawerQueueBootstrap && opportunityRecordChromePending))
                            )
                        }
