...
function StoryBulletRow({ bullet }: { bullet: OutcomeStoryBullet }) {
    ...
    return (
        <li className={clsx(opGroupedRowInner, "flex gap-2")} data-testid="form-outcome-story-bullet">
            <span className={clsx("mt-0.5 shrink-0 text-sm font-semibold", toneClass)} aria-hidden>
                {icon}
            </span>
            <span className="text-sm text-alloy-midnight">{bullet.text}</span>
        </li>
    );
}
    ...
    return (
        <li className={opGroupedRowInner}>
            <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
                ...
                {status === "varies" ?
                    <span className={clsx("text-xs font-semibold uppercase tracking-wide", "text-amber-700")}>
                        Varies
                    </span>
                :   null}
            </div>
            <p className={clsx("mt-0.5", value === "Not configured yet" ? opMutedMeta : opMetadata)}>{value}</p>
        </li>
    );
}
...
export function FormOutcomeConfigPanel({
    ...
    useEffect(() => {
        void loadLabels();
    }, [loadLabels, links.length]);

    useEffect(() => {
        if (!selectedLinkId && defaultSelectedLinkId) {
            setSelectedLinkId(defaultSelectedLinkId);
        }
    }, [defaultSelectedLinkId, selectedLinkId]);

    const effectiveSelectedLinkId = selectedLinkId || defaultSelectedLinkId;
    ...
    return (
        ...
        >
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h2 className={opSectionTitle}>Operational Outcome</h2>
                    <p className={opSectionSupport}>What happens when this form is submitted.</p>
                </div>
            ...
            {selectedLinkModel?.varianceCallout ?
                <div
                    className="mt-3 rounded-lg border border-amber-200/80 bg-amber-50/90 px-3 py-2.5"
                    data-testid="form-outcome-variance-callout"
                >
                    <p className="text-sm font-semibold text-amber-900">{selectedLinkModel.varianceCallout.title}</p>
                    <p className={clsx("mt-1", opMetadata)}>{selectedLinkModel.varianceCallout.body}</p>
                </div>
            :   null}

            {!editing && selectedLinkModel && selectedLinkModel.whenSubmittedStory.length > 0 ?
                <div className="mt-4" data-testid="form-outcome-when-submitted-story">
                    <h3 className="text-sm font-semibold text-alloy-midnight">When this form is submitted</h3>
            ...
            :   <div className="mt-4 space-y-4" data-testid="form-outcome-read-mode">
