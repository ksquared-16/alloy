#!/usr/bin/env npx tsx
/**
 * Backfill placement_candidates from waitlisted opportunities (Phase 2 — Card 2).
 *
 * Env:
 *   ORG_ID=uuid              (required)
 *   DRY_RUN=1                (optional — counts only)
 *   LIMIT=500                (optional)
 *   WAIT_SINCE_FALLBACK_CREATED_AT=1
 *
 * Run from repo `web/`:
 *   ORG_ID=<uuid> npm run dev:backfill:placement-candidates
 *   ORG_ID=<uuid> DRY_RUN=1 npm run dev:backfill:placement-candidates
 *
 * Safe default: ORG_ID required; omit DRY_RUN to apply writes (use DRY_RUN=1 first).
 */

import { config as loadEnv } from "dotenv";
import { resolve } from "path";
import { createAdminClient } from "@/lib/supabaseAdmin";
import { runPlacementCandidateBackfill } from "@/lib/orchestration/placement/backfill/placementCandidateBackfill";

loadEnv({ path: resolve(process.cwd(), ".env.local") });
loadEnv({ path: resolve(process.cwd(), ".env") });

async function main() {
    const orgId = (process.env.ORG_ID ?? process.env.DEV_QUEUE_ORG_ID ?? "").trim();
    if (!orgId) {
        console.error("ORG_ID or DEV_QUEUE_ORG_ID is required");
        process.exit(1);
    }

    const dryRun = process.env.DRY_RUN === "1" || process.env.DRY_RUN === "true";
    const limitRaw = process.env.LIMIT?.trim();
    const limit = limitRaw ? Number.parseInt(limitRaw, 10) : undefined;
    const waitSinceFallbackCreatedAt =
        process.env.WAIT_SINCE_FALLBACK_CREATED_AT === "1" ||
        process.env.WAIT_SINCE_FALLBACK_CREATED_AT === "true";

    const supabase = createAdminClient();
    const { counts, error_messages } = await runPlacementCandidateBackfill(supabase, {
        orgId,
        dryRun,
        limit: Number.isFinite(limit) ? limit : undefined,
        waitSinceFallbackCreatedAt,
    });

    console.log(JSON.stringify({ dry_run: dryRun, org_id: orgId, counts, error_messages }, null, 2));
    if (counts.errors > 0) process.exit(1);
}

main().catch((e) => {
    console.error(e);
    process.exit(1);
});
