...
export async function findOrCreatePersonInOrg(
  ...
  if (emailNorm) {
    ...
    if (byEmail && typeof (byEmail as { id?: string }).id === "string") return (byEmail as { id: string }).id;
  }
  if (phoneNorm) {
    let q = supabase.from("persons").select("id").eq("phone", phoneNorm).limit(1);
    if (org_id) q = q.eq("org_id", org_id);
    const { data: byPhone } = await q.maybeSingle();
    if (byPhone && typeof (byPhone as { id?: string }).id === "string") return (byPhone as { id: string }).id;
  }
  ...
  if (error || !created) {
    if (error?.code === "23505") {
      if (emailNorm) {
        ...
        if (again && typeof (again as { id?: string }).id === "string") return (again as { id: string }).id;
      }
      if (phoneNorm) {
