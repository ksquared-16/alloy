...
export function buildFormIntakeMetaFromPayload(input: {
    ...
    const childFirst = readTrimmedString(input.values, paths.child_first_name);
    const childLast = readTrimmedString(input.values, paths.child_last_name);
    const childDobRaw = readTrimmedString(input.values, paths.child_dob);
    const childDobValid =
        childDobRaw && /^\d{4}-\d{2}-\d{2}$/.test(childDobRaw) ? childDobRaw : null;

    const child =
        childFirst || childLast || childDobValid
            ? {
                  first_name: childFirst,
                  last_name: childLast,
                  dob: childDobValid,
                  display_name:
                      [childFirst, childLast].filter(Boolean).join(" ").trim() ||
                      childFirst ||
                      childLast ||
                      null,
              }
    ...
    const intake: FormIntakeMeta = {
        ...
        child: child?.first_name || child?.last_name || child?.dob ? child : undefined,
