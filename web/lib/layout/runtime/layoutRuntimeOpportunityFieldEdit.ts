...
function normalizeOpportunityLocationIdPatchValue(value: string): string | null {
    const t = value.trim();
    if (!t) return null;
    if (!isUuidLike(t)) return null;
    return t;
}
...
export function buildLayoutRuntimeOpportunityNativePatch(
    ...
    for (const refKey of LAYOUT_RUNTIME_OPPORTUNITY_NATIVE_EDITABLE_REF_KEYS) {
        ...
        if (bodyKey === "location_id") {
            const normalized = normalizeOpportunityLocationIdPatchValue(draft[refKey] ?? "");
            const baselineNormalized = normalizeOpportunityLocationIdPatchValue(baseline[refKey] ?? "");
            if (normalized === baselineNormalized) continue;
            patch[bodyKey] = normalized;
            continue;
        }
...
export async function saveLayoutRuntimeOpportunityNativeEdits(input: {
    ...
    if (Object.keys(body).length === 0) {
        const draftLocation = String(input.draft["opportunity.location_id"] ?? "").trim();
        const baselineLocation = String(input.baseline["opportunity.location_id"] ?? "").trim();
        if (draftLocation !== baselineLocation && draftLocation && !isUuidLike(draftLocation)) {
            return { ok: false, error: "Select a valid school location before saving." };
        }
        return { ok: true };
    }
    if (body.location_id != null && !isUuidLike(String(body.location_id))) {
        return { ok: false, error: "Select a valid school location before saving." };
    }
