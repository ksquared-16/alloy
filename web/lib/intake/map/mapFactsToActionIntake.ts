...
} from "@/lib/intake/map/adapters/mapHouseholdToCreateLeadFields";
...
function mapCreateLeadHouseholdToFields(input: {
    extraction: IntakeFactExtractionResult;
    spec: ActionIntakeSpec;
...
/** Map extracted facts to action intake candidates. Create Lead uses adapter bindings; extend for other actions. */
export function mapFactsToActionIntake(input: {
    ...
    if (input.spec.action_key === "create_lead") {
        return mapCreateLeadHouseholdToFields(input);
    }
    return mapCreateLeadHouseholdToFields(input);
}
