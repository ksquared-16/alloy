...
function mapOptionRows(rows: PersonStatusOptionRow[]): StatusOptionVm[] {
    return rows
        .filter((r) => r.is_active !== false)
        .map((r) => ({
            status_key: r.status_key,
            label: trimOrNull(r.status_label) ?? r.status_key,
...
export function buildPersonDrawerStatusControlVm(params: {
    ...
    const label = trimOrNull(params.record._status_display) ?? statusKey ?? "—";

    const activeRows: PersonStatusOptionRow[] = params.statusDefs
    ...
    const options = mapOptionRows(withLegacy);
