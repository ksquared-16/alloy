    ...
    for (const [kind, kindRefs] of refsByKind) {
        ...
        for (const ref of kindRefs) {
            ...
            out.set(
                ...
                label
                    ? { kind, id: ref.id, label: label.label, originalFilename: label.originalFilename ?? null, ocr: label.ocr ?? null, receivedAt: label.receivedAt, channel: label.channel, resolved: true }
