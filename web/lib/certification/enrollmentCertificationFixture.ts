...
/*
 * RESET LIVES HERE, not in the seed script.
 *
 * The certification driver has to be able to return the fixture to a known state at the start of
 * every invocation, and a reset that only the CLI can reach forces the driver to shell out or to
 * grow a second implementation. Two implementations of a destructive boundary is exactly the shape
 * that produced the leak this reset was repaired for, so there is one, and both callers use it.
 */
/**
 ...
 */
export async function removeEnrollmentCertificationFixture(
    supabase: SupabaseClient,
    orgId: string,
