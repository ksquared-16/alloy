const MIN_LEN = 2;
const MAX_LEN = 64;
const STATUS_KEY_MAX_LEN = 32;

/** Status definition keys — 2–32 chars, lowercase + underscores. */
export const STATUS_KEY_REGEX = /^[a-z0-9_]{2,32}$/;

/** Slug for status_definitions.status_key from display label. */
export function slugifyStatusKey(label: string): string {
    let s = label
        ...
        .replace(/_+/g, "_")
        .replace(/^_|_$/g, "");
    if (s.length < MIN_LEN) s = "status";
    if (s.length > STATUS_KEY_MAX_LEN) {
        s = s.slice(0, STATUS_KEY_MAX_LEN).replace(/_+$/g, "");
        if (s.length < MIN_LEN) s = "status";
    }
    return s;
}

/** Unique status key within an entity type's existing keys. */
export function uniqueStatusKey(baseLabel: string, reserved: Set<string>): string {
    const b0 = slugifyStatusKey(baseLabel);
    const ok = (k: string) => STATUS_KEY_REGEX.test(k) && !reserved.has(k);
    if (ok(b0)) return b0;
    for (let n = 2; n < 10000; n++) {
        const suffix = `_${n}`;
        let stem = b0;
        if (stem.length + suffix.length > STATUS_KEY_MAX_LEN) {
            stem = stem.slice(0, STATUS_KEY_MAX_LEN - suffix.length).replace(/_+$/g, "");
            if (stem.length < MIN_LEN) stem = "st";
        }
