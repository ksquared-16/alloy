...
const runtimeRenderer: SurfaceRuntimeRenderer = {
    renderCard: (instance, ctx) => {
        ...
        // Compact (header) cells: Operational Answer style — label / answer+dot / value.
        // Matches the runtime OperationalAnswerStrip exactly so preview = production.
        if (ctx.density === "compact") {
            const ANSWER_TEXT: Record<SampleTone, string> = {
                healthy: "On track", warning: "Needs attention", critical: "Action required", neutral: "Live",
            };
            const DOT_COLOR: Record<SampleTone, string> = {
                healthy: "#00A283", warning: "#BC4300", critical: "#7C273A", neutral: "#7C8FA6",
            };
            const dotColor = DOT_COLOR[s.tone];
            const answerText = hasContent ? ANSWER_TEXT[s.tone] : "Configure";
            const valueText = hasContent ? (s.unit ? `${s.value} ${s.unit}` : s.value) : "—";
            return (
                <div
                    className={`flex min-h-[50px] flex-col justify-center border-r border-alloy-stone/10 px-3 ${dimmed ? "opacity-50" : ""}`}
                    data-metric-visual={`metric_${renderer}`}
                >
                    <p className="text-[9px] font-medium uppercase tracking-wide text-alloy-midnight/40">{title}</p>
                    <p className="mt-0.5 flex items-center gap-1.5 text-[13px] font-medium" style={{ color: dotColor }}>
                        <span className="inline-block h-[5px] w-[5px] shrink-0 rounded-full" style={{ background: dotColor }} aria-hidden="true" />
                        {answerText}
                    </p>
                    <p className="mt-0.5 text-[12px] font-semibold text-alloy-midnight">{valueText}</p>
                </div>
            );
        }
