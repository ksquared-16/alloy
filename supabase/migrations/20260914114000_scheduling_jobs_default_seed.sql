-- A NEW ORGANIZATION WOULD STILL HAVE SEEDED THE TWO KEYS TO ops.
--
-- `20260914113000` catalogued `fin.post`, granted the three keys to every existing administrator,
-- and removed the two write keys from ops wherever no audited change had ever touched them. It did
-- not touch `seed_default_rbac`, which is the function the `orgs` insert trigger calls — so an
-- organization created afterwards would be born with exactly the default the slice just corrected,
-- and enforcing the keys would hand its ops role ten operations the role-title gate denied.
--
-- ── WHAT CHANGES ──
--
-- ADMIN gains `fin.post`, on the contract that region already states: the Organization
-- Administrator receives every ordinary organizational capability the platform defines.
--
-- OPS LOSES `scheduling.write` and `ops.jobs.write`, and never receives `fin.post`. The withheld
-- set grows from fourteen to seventeen. This is not a new judgement about what ops should be — it
-- is what ops could actually DO before this program touched anything: all fourteen routes answered
-- ops with 403, because the gate read `ctx.role !== "admin"`. The keys were seeded and enforced
-- nowhere, so the grant conferred nothing. Keeping it while the keys became real would have been
-- the widening, not removing it.
--
-- An organization remains free to grant any of the three to its ops role, or to any role it
-- defines, through /organization/access. A default package is not a forced package.
--
-- The catalog literal inside the function is untouched: it reproduces what `permission_definitions`
-- held on 2026-07-29, and `20260914113000` is where `fin.post` enters the catalog.
...
BEGIN
    ...
    IF strpos(v_src, 'W12:ADMIN-GRANTS:BEGIN') = 0 OR strpos(v_src, 'W12:OPS-GRANTS:BEGIN') = 0
       OR strpos(v_src, 'ACCESSV2:DIRECTOR-GRANTS:BEGIN') = 0 THEN
        RAISE EXCEPTION
            'SCHEDJOBS ABORT: the installed seed_default_rbac does not carry the grant-enumeration sentinels. Nothing can be asserted about what it grants, so the migration refuses to leave it installed.';
    END IF;
    ...
    SELECT array_agg(k ORDER BY k) INTO v_missing
      FROM unnest(ARRAY['processing.operate', 'processing.archive',
                        ...
                        'scheduling.write', 'ops.jobs.write', 'fin.post']) AS k
     WHERE strpos(v_admin_region, '''' || k || '''') = 0;
    IF v_missing IS NOT NULL THEN
        RAISE EXCEPTION 'SCHEDJOBS ABORT: % absent from the admin enumeration; a new administrator could not work Processing.', v_missing;
    END IF;
    ...
    IF v_ops_extra IS NOT NULL THEN
        RAISE EXCEPTION
            'SCHEDJOBS ABORT: the ops enumeration grants %, which the migration that introduced each of those keys explicitly withheld from ops. This would widen ops, not preserve it.',
            v_ops_extra;
    END IF;

    IF strpos(v_ops_region, '''forms.submissions.confirm''') = 0 THEN
        RAISE EXCEPTION 'SCHEDJOBS ABORT: ops lost forms.submissions.confirm, the one Forms write it already had.';
    END IF;
END
$formsguard$;
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    VALUES (v_org, 'Schedules+Jobs seed self-test', '_sj_seed_' || substr(v_org::text, 1, 8));
    ...
    SELECT count(*) INTO v_admin FROM public.role_permission_grants
     WHERE org_id = v_org AND role_key = 'admin' AND allowed
       AND permission_key IN ('scheduling.write', 'ops.jobs.write', 'fin.post');
    IF v_admin <> 3 THEN
        RAISE EXCEPTION 'SCHEDJOBS SEED ABORT: a new administrator holds % of the 3 keys, expected 3.', v_admin;
    END IF;

    -- The correction this slice exists for: a NEW organization must not seed these to ops.
    ...
    IF v_ops_wide <> 0 THEN
        RAISE EXCEPTION 'SCHEDJOBS SEED ABORT: a new ops role received % key(s) the default package withholds.', v_ops_wide;
    END IF;

    /*
     * The previous two slices are re-proved here, not assumed. This function is redefined by
     * whichever slice last needed to teach it a key, and each redefinition reproduces the whole
     * enumeration — precisely the operation that drops a line by accident.
     */
    SELECT count(*) INTO v_proc FROM public.role_permission_grants
    ...
    IF v_proc <> 4 OR v_forms <> 3 THEN
        RAISE EXCEPTION 'SCHEDJOBS SEED ABORT: regression — processing=% (expected 4), forms=% (expected 3).', v_proc, v_forms;
    END IF;

    SELECT string_agg(k.key, ', ') INTO v_missing
      FROM (VALUES ('scheduling.write'), ('ops.jobs.write'), ('fin.post')) AS k(key)
    ...
    IF v_missing IS NOT NULL THEN
        RAISE EXCEPTION 'SCHEDJOBS SEED ABORT: a new administrator is missing %.', v_missing;
    END IF;
    ...
    RAISE NOTICE 'SCHEDJOBS: a new organization gives its administrator all three keys, its ops role none of them, and the Forms and Processing packages are intact.';
END
$sjseed$;
