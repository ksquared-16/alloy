-- A NEW ORGANIZATION WOULD HAVE NO SENSITIVE CONFIGURATION AUTHORITY AT ALL.
--
-- `20260914183000` catalogued option_sets.delete, layouts.lifecycle and fields.delete and granted
-- them to every existing administrator. It did not touch `seed_default_rbac`, which is the function
-- the `orgs` insert trigger calls — so an organization created afterwards would have an
-- administrator who could not delete an option set, publish a layout or remove a field, while the
-- routes now demand exactly those keys. That is the cliff `20260910183000` was written to end.
--
-- ── WHAT CHANGES, AND WHAT DELIBERATELY DOES NOT ──
--
-- ADMIN gains the three sensitive keys, on the contract that region already states.
--
-- OPS gains none of them — and KEEPS option_sets.manage, layouts.manage and fields.manage, which it
-- has held all along. That distinction is the whole slice: the route convergence is justified BY the
-- fact that ops already holds the manage keys and already exercises equivalent authority through
-- Config Layout Assist. A seed that dropped them would remove the very authority the convergence is
-- aligning to, and would break the assisted path for every new organization.
--
-- The withheld set grows from seventeen to twenty. Every exclusion is the product's own boundary:
-- all six of those operations answered ops with 403 before this program touched anything.
--
-- Nothing is granted to school_director or regional_lead. The catalog literal inside the function is
-- untouched; `20260914183000` is where these keys enter the catalog.
...
BEGIN
    ...
    IF strpos(v_src, 'W12:ADMIN-GRANTS:BEGIN') = 0 OR strpos(v_src, 'W12:OPS-GRANTS:BEGIN') = 0
       OR strpos(v_src, 'ACCESSV2:DIRECTOR-GRANTS:BEGIN') = 0 THEN
        RAISE EXCEPTION
            'CONFIGAUTH ABORT: the installed seed_default_rbac does not carry the grant-enumeration sentinels. Nothing can be asserted about what it grants, so the migration refuses to leave it installed.';
    END IF;
    ...
    SELECT array_agg(k ORDER BY k) INTO v_missing
      FROM unnest(ARRAY['processing.operate', 'processing.archive',
                        ...
                        'option_sets.delete', 'layouts.lifecycle', 'fields.delete']) AS k
     WHERE strpos(v_admin_region, '''' || k || '''') = 0;
    IF v_missing IS NOT NULL THEN
        RAISE EXCEPTION 'CONFIGAUTH ABORT: % absent from the admin enumeration; a new administrator could not work Processing.', v_missing;
    END IF;
    ...
    IF v_ops_extra IS NOT NULL THEN
        RAISE EXCEPTION
            'CONFIGAUTH ABORT: the ops enumeration grants %, which the migration that introduced each of those keys explicitly withheld from ops. This would widen ops, not preserve it.',
            v_ops_extra;
    END IF;

    IF strpos(v_ops_region, '''forms.submissions.confirm''') = 0 THEN
        RAISE EXCEPTION 'CONFIGAUTH ABORT: ops lost forms.submissions.confirm, the one Forms write it already had.';
    END IF;
END
$formsguard$;
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    VALUES (v_org, 'Configuration authority seed self-test', '_cfg_seed_' || substr(v_org::text, 1, 8));
    ...
    IF v_admin <> 3 THEN
        RAISE EXCEPTION 'CONFIGAUTH SEED ABORT: a new administrator holds % of the 3 sensitive keys, expected 3.', v_admin;
    END IF;
    ...
    SELECT count(*) INTO v_ops_new FROM public.role_permission_grants
     WHERE org_id = v_org AND role_key = 'ops' AND allowed
       AND permission_key IN ('option_sets.delete', 'layouts.lifecycle', 'fields.delete');
    IF v_ops_new <> 0 THEN
        RAISE EXCEPTION 'CONFIGAUTH SEED ABORT: a new ops role received % sensitive key(s) the default package withholds.', v_ops_new;
    END IF;

    /*
     * ops MUST still receive the three manage keys. The route convergence assumes ops already holds
     * them — that is the whole basis for calling it path convergence rather than an expansion — and
     * a seed that quietly dropped them would break Config Layout Assist for every new organization.
     */
    SELECT count(*) INTO v_ops_keep FROM public.role_permission_grants
     WHERE org_id = v_org AND role_key = 'ops' AND allowed
       AND permission_key IN ('option_sets.manage', 'layouts.manage', 'fields.manage');
    IF v_ops_keep <> 3 THEN
        RAISE EXCEPTION 'CONFIGAUTH SEED ABORT: a new ops role holds % of the 3 manage keys, expected 3.', v_ops_keep;
    END IF;

    -- The three prior slices are re-proved, not assumed: each redefinition reproduces the whole
    -- enumeration, which is exactly the operation that drops a line by accident.
    SELECT count(*) INTO v_prior FROM public.role_permission_grants
     WHERE org_id = v_org AND role_key = 'admin' AND allowed
       AND (permission_key LIKE 'forms.%' OR permission_key LIKE 'processing.%'
            OR permission_key IN ('scheduling.write', 'ops.jobs.write', 'fin.post'));
    IF v_prior <> 10 THEN
        RAISE EXCEPTION 'CONFIGAUTH SEED ABORT: prior-slice packages regressed to % keys, expected 10.', v_prior;
    END IF;

    SELECT string_agg(k.key, ', ') INTO v_missing
      FROM (VALUES ('option_sets.delete'), ('layouts.lifecycle'), ('fields.delete')) AS k(key)
    ...
    IF v_missing IS NOT NULL THEN
        RAISE EXCEPTION 'CONFIGAUTH SEED ABORT: a new administrator is missing %.', v_missing;
    END IF;
    ...
    RAISE NOTICE 'CONFIGAUTH: a new organization gives its administrator all three sensitive keys, its ops role none of them and all three manage keys, and the prior packages are intact.';
END
$cfgseed$;
