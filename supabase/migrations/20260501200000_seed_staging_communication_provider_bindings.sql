...
-- Guarded: communication_provider_bindings.org_id is NOT NULL REFERENCES public.orgs.
-- The staging org 93667019-… does not exist on a fresh `supabase db reset`, so these
-- literal-org inserts would violate the FK. No-op when the org is absent; behavior is
-- unchanged on staging where the org exists.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.orgs WHERE id = '93667019-bd28-49b5-a688-acc9bb1e0a19'::uuid) THEN
    RAISE NOTICE 'Staging org 93667019-bd28-49b5-a688-acc9bb1e0a19 absent; skipping communication provider binding seed.';
    RETURN;
  END IF;
