-- assignments table only. assignment_statuses already exists (assigned, accepted, declined, removed, completed).
...
COMMENT ON TABLE public.assignments IS 'Vendor assignment per schedule occurrence; one active assignment per schedule. Uses existing assignment_statuses (e.g. assigned, accepted, declined).';
