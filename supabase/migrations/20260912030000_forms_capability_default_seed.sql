-- A NEW ORGANIZATION'S ADMINISTRATOR COULD NOT USE FORMS.
--
-- `20260912020000` defined the three Forms capabilities and granted them to the roles of every
...
-- four roles, the pre-Forms grant set, and no Forms authority at all.
--
-- That is exactly the defect `20260910183000` was written to end: "an organization inherits a key if
-- and only if it existed on the day that key was catalogued." A backfill that does not also teach
-- the seed only moves the cliff to a later date. The completeness lock caught it — a freshly
-- bootstrapped administrator held 66 catalogued capabilities instead of all of them.
--
-- ── WHAT CHANGES, AND WHAT DELIBERATELY DOES NOT ──
--
-- The three keys join the ADMIN enumeration, on the contract that region already states: the
-- Organization Administrator receives every ordinary organizational capability the platform defines.
--
-- They do NOT join the catalog literal inside the function. That literal is a reproduction of what
-- `permission_definitions` held on 2026-07-29, carried forward unchanged; widening it would quietly
-- rewrite history, and `20260912020000` is where these keys enter the catalog.
--
-- `ops` receives `forms.submissions.confirm` and NOT the other two, so the withheld set grows from
-- nine to eleven. That is not a new judgement. Before this program, `confirm-linkage` was the single
-- Forms write ops could reach — "operator confirms auto-linked CRM rows are correct (payload.meta
-- only; no CRM mutation)" — while `manual-link`, which sets the CRM keys by hand, refused them.
-- Both exclusions are the product's own boundary, preserved.
--
...
BEGIN
    ...
    IF strpos(v_src, 'W12:ADMIN-GRANTS:BEGIN') = 0 OR strpos(v_src, 'W12:OPS-GRANTS:BEGIN') = 0
       OR strpos(v_src, 'ACCESSV2:DIRECTOR-GRANTS:BEGIN') = 0 THEN
        RAISE EXCEPTION
            'FORMS ABORT: the installed seed_default_rbac does not carry the grant-enumeration sentinels. Nothing can be asserted about what it grants, so the migration refuses to leave it installed.';
    END IF;
    ...
    SELECT array_agg(k ORDER BY k) INTO v_missing
      FROM unnest(ARRAY['forms.author', 'forms.submissions', 'forms.submissions.confirm']) AS k
     WHERE strpos(v_admin_region, '''' || k || '''') = 0;
    IF v_missing IS NOT NULL THEN
        RAISE EXCEPTION 'FORMS ABORT: % absent from the admin enumeration; a new administrator could not use Forms.', v_missing;
    END IF;
    ...
    IF strpos(v_ops_region, '''forms.submissions.confirm''') = 0 THEN
        RAISE EXCEPTION 'FORMS ABORT: ops lost forms.submissions.confirm, the one Forms write it already had.';
    END IF;
END
$formsguard$;
-- ─────────────────────────────────────────────────────────────────────────────
...
DECLARE
    ...
    v_missing  text;
BEGIN
    INSERT INTO public.orgs (id, name, slug) VALUES (v_org, 'Forms seed self-test', '_forms_seed_' || substr(v_org::text, 1, 8));
    ...
    IF v_admin <> 3 THEN
        RAISE EXCEPTION 'FORMS SEED ABORT: a new administrator holds % Forms capabilities, expected 3.', v_admin;
    END IF;

    SELECT count(*) INTO v_ops_conf FROM public.role_permission_grants
     WHERE org_id = v_org AND role_key = 'ops' AND permission_key = 'forms.submissions.confirm' AND allowed;
    SELECT count(*) INTO v_ops_wide FROM public.role_permission_grants
     WHERE org_id = v_org AND role_key = 'ops' AND permission_key IN ('forms.author', 'forms.submissions') AND allowed;
    IF v_ops_conf <> 1 OR v_ops_wide <> 0 THEN
        RAISE EXCEPTION 'FORMS SEED ABORT: ops got confirm=% wide=%, expected 1 and 0.', v_ops_conf, v_ops_wide;
    END IF;

    /*
     * Scoped to the FORMS keys on purpose.
     *
     * The first draft asserted the whole catalogue and failed on `attendance.record.assigned_only`,
     * `integrations.manage` and `integrations.read` — three keys catalogued by other programs whose
     * migrations seeded the organizations existing at the time and never taught this function. That
     * is the same drift `20260910183000` named, still open, and owned by those programs. Repairing
     * it here would mean this Forms migration silently granting three unrelated authorities to every
     * administrator, which is not a thing a Forms slice should decide. It is reported as debt instead.
     */
    SELECT string_agg(k.key, ', ') INTO v_missing
     ...
     WHERE NOT EXISTS (SELECT 1 FROM public.role_permission_grants g
                        WHERE g.org_id = v_org AND g.role_key = 'admin' AND g.permission_key = k.key AND g.allowed);
    IF v_missing IS NOT NULL THEN
        RAISE EXCEPTION 'FORMS SEED ABORT: a new administrator is missing %.', v_missing;
    END IF;
    ...
    RAISE NOTICE 'FORMS: a newly bootstrapped organization receives the whole catalogue, and ops keeps exactly its confirmation.';
END
$formsseed$;
